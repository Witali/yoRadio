...
# C context: {
# Decoder-only quant_partition: ctx.encode is immutable zero in this radio.
...
# decoder-only: encode=0 branch removed
...
# @OPUS@\upstream\celt\bands.c:673:       qn = (qn+1)>>1<<1;
	...
	s32i.n	a3, sp, 28	# %sfp,
# @OPUS@\upstream\celt\bands.c:752:    if (encode)
	j	.L128	# decoder-only: encode=0, same successor
# decoder-only: unreachable instruction removed
.L84:
