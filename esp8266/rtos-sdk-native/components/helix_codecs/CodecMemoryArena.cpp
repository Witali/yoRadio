...
namespace {
constexpr size_t kMp3SubbandBytes = 8708U;
uint8_t *s_arena;
void *s_mp3_subband;
bool s_fragmented;
size_t s_capacity;
...
bool CodecArenaPreallocateMp3(void) {
    if (s_mp3_subband) return true;
    s_mp3_subband = heap_caps_calloc(1, kMp3SubbandBytes,
                                    MALLOC_CAP_32BIT);
    if (!s_mp3_subband) {
        ESP_LOGE("codec_arena",
                 "Cannot reserve %u-byte MP3 synthesis buffer; 32-bit free %u",
                 (unsigned)kMp3SubbandBytes,
                 (unsigned)heap_caps_get_free_size(MALLOC_CAP_32BIT));
        return false;
    }
    ESP_LOGI("codec_arena", "Reserved %u-byte MP3 synthesis buffer early",
             (unsigned)kMp3SubbandBytes);
    return true;
}

size_t CodecArenaPreallocatedBytes(void) {
    return s_mp3_subband ? kMp3SubbandBytes : 0U;
}
...
void *CodecArenaCalloc(CodecArenaOwner owner, size_t count, size_t size) {
    ...
    bool use_mp3_reserve = s_fragmented && owner == CODEC_ARENA_MP3 &&
                           bytes == kMp3SubbandBytes && s_mp3_subband;
    if (s_fragmented && owner == CODEC_ARENA_MP3 &&
        bytes == kMp3SubbandBytes)
        caps = MALLOC_CAP_32BIT;
    void *result = use_mp3_reserve
        ? s_mp3_subband
        : (s_fragmented ? heap_caps_calloc(count, size, caps)
    ...
    if (!s_fragmented || use_mp3_reserve) memset(result, 0, bytes);
    s_used = offset + bytes;
    return result;
}

void CodecArenaFree(void *pointer) {
    if (pointer == s_mp3_subband) return;
