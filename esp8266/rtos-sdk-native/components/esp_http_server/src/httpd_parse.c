...
static esp_err_t cb_headers_complete(http_parser *parser)
{
    ...
    if (parser->upgrade) {
        ...
        if (httpd_req_get_hdr_value_str(r, "Upgrade", ws_upgrade_hdr_val,
                                        sizeof(ws_upgrade_hdr_val)) != ESP_OK ||
            strcasecmp("websocket", ws_upgrade_hdr_val) != 0) {
            parser_data->error = HTTPD_400_BAD_REQUEST;
            parser_data->status = PARSING_FAILED;
            return ESP_FAIL;
        }
...
esp_err_t httpd_req_new(struct httpd_data *hd, struct sock_db *sd)
{
    ...
    if (sd->ws_handshake_done && sd->ws_handler != NULL) {
        ...
        if (err == ESP_OK && ra->ws_type < HTTPD_WS_TYPE_CLOSE) {
            err = sd->ws_handler(r);
        }
        if (err != ESP_OK) {
            httpd_req_cleanup(r);
