// Phase-specialized signed exp2_table8 access on accepted index-half ASM; not production.
...
const sem=require('./pvq_exp2_word_proof.cjs');
...
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-exp2-word-'+t+'-v1';
...
const dependencies=['pvq_index_word_proof.cjs','pvq_index_half_proof.cjs','pvq_exp2_word_proof.cjs','pvq_exp2_word.s','pvq_endpoint_word_proof.cjs','frozen_reloads.cjs','pvq_row_word_proof.cjs','pvq_a4_word_proof.cjs','pvq_byte_word_proof.cjs','bits_fifth_proof.cjs','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs','report_layout.cjs'];
...
function sourceFor(){return fs.readFileSync(path.join(__dirname,'pvq_exp2_word.s'),'utf8').replace(/\r\n/g,'\n');}
function prove(functions,actual,patches){
 const search=sem.prove(functions.quant_partition,actual.quant_partition);
 const contract={scope:'Unchanged caller chain and frame. Phase-specific exp2_table8 helper preserves every register except original signed a3 result, including a0/a10/SAR. Parent private decoder contract retained.'};
 for(const n of names.slice(1))assert.deepEqual(actual[n],functions[n]);
 return{search,contract};
}
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_exp2_word_variant,'control');assert.equal(b.post_link_pvq_exp2_word_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_exp2_word_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){
...
function tableProof(elf) {
 ...
 assert.equal(readAt(0x40211a9c,4).readUInt32LE(),sem.tableBase,'exp2_table8 literal pointer');
 const raw=readAt(sem.tableBase,sem.tableBytes),values=sem.tableValues.map((_,i)=>raw.readInt16LE(i*2));assert.deepEqual(values,sem.tableValues);
 assert.equal(sem.tableBase%4,0);assert.equal(raw.length,16);
 return {...inherited,exp2_address:sem.tableBase,exp2_bytes:raw.length,exp2_sha256:hash(raw),exp2_word_bytes:raw.length};
}
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' signed exp2_table8 word helper ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_exp2_word_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
...
// Eight int16 entries occupy four aligned words; every read stays inside the table.
function cModels(){
 const dir=path.join(root,'.build/opus-bands-pvq-exp2-word');fs.mkdirSync(dir,{recursive:true});
...
static int y_pvq_exp2_word(const opus_int16 *table,int i) {
  opus_uint32 word=0;
  celt_assert(i>=0 && i<8);
  const unsigned offset=(unsigned)i*2,aligned=offset&~3u;
  memcpy(&word,(const unsigned char*)table+aligned,4);
...
`;
  ...
  text=once(text,'qn = exp2_table8[qb&0x7]>>(14-(qb>>BITRES));',
   'qn = (stereo ? exp2_table8[qb&0x7] : y_pvq_exp2_word(exp2_table8,qb&0x7))>>(14-(qb>>BITRES));');
