...
const base=require('./frozen_reloads.cjs'),previous=require('./pvq_byte_word.cjs');
const sem=require('./pvq_a4_word_proof.cjs');
...
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-a4-word-'+t+'-v1';
...
const dependencies=['pvq_a4_word_proof.cjs','pvq_a4_word.s','pvq_byte_word_proof.cjs','bits_fifth_proof.cjs','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs','report_layout.cjs'];
...
function sourceFor(){return fs.readFileSync(path.join(__dirname,'pvq_a4_word.s'),'utf8').replace(/\r\n/g,'\n');}
...
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_a4_word_variant,'control');assert.equal(b.post_link_pvq_a4_word_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_a4_word_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){
  assert.equal(m.post_link_pvq_byte_word_variant,'candidate');assert.equal(m.post_link_endpoint_cost_variant,'candidate');assert.equal(m.post_link_bits_fourth_variant,'candidate');assert.equal(m.post_link_bits_fifth_variant,'candidate');assert.equal(m.post_link_pvq_row_loop_variant,'candidate');assert.equal(m.post_link_post_pair_variant,'candidate');
...
function generate(){
 ...
 patches.forEach((p,i)=>{const s=ss.find(s=>s.name==='.text.patch'+i);assert.equal(s.address,p.address);assert.equal(s.bytes,p.bytes);
  p.after_hex=object.subarray(s.offset,s.offset+s.bytes).toString('hex');const off=base.offsetAt(a,p.address,p.bytes);p.before_hex=a.subarray(off,off+p.bytes).toString('hex');if(i===0)assert.equal(p.before_hex,'00'.repeat(p.bytes),'New helper must occupy zero padding');});
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' word-load PVQ byte helper ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_a4_word_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
...
function cModels(){
 const dir=path.join(root,'.build/opus-bands-pvq-a4-word');fs.mkdirSync(dir,{recursive:true});
 const {once}=require('./bands.cjs');
 return previous.cModels().map(model=>{
  let text=fs.readFileSync(model.file,'utf8').replace(/\r\n/g,'\n');
  text=once(text,'i==0?cache[mid]:y_pvq_word8(m,cache+mid)','y_pvq_word8(m,cache+mid)');
  text=once(text,'LM != -1 && b > cache[cache[0]]+12 && N>2','LM != -1 && b > y_pvq_word8(m,cache+cache[0])+12 && N>2');
