...
const {root,hash,sourceHash}=require('./export.cjs'),f=require('./pvq_n3_unroll4.cjs'),sem=require('./pvq_n3_unroll4_proof.cjs'),base=require('./frozen_reloads.cjs');
function analyze(){
 ...
 for(const c of census.cases){
  ...
  const scalar=f.read(path.join(root,'firmware/development/esp8266-opus-pvq-n3-diff-candidate-v1/instruction-census.json')).cases.find(v=>v.name===c.name);
  const hypothesis=f.read(path.join(root,'docs/benchmarks/esp8266-opus-pvq-n3-unroll4-2026-09-19/summary.json')).cases.find(v=>v.name===c.name);
  assert.equal(newSteps,scalar.new_instructions-hypothesis.reduction);
