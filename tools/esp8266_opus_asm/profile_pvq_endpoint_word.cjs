// Frequency and word bounds of the remaining endpoint-cost byte probe; no target modification.
...
const endpointAnchor=' const int lower=lo==0?-1:(int)cache[lo],upper=(int)cache[hi];';
function instrument(text,observer) {
 text=once(text,'static int y_endpoint_cost(',observer+'\nstatic int y_endpoint_cost(');
 return once(text,endpointAnchor,' const int lower=lo==0?-1:y_endpoint_probe(m,cache+lo,0),upper=y_endpoint_probe(m,cache+hi,1);');
}
function validateCounts(c) {
 assert.equal(c.phase.length,2);assert.ok(c.lower<=c.upper);
 for(const [i,key]of['lower','upper'].entries()){
  assert.equal(c.phase[i].length,4);
  for(const v of[c[key],...c.phase[i]])assert.ok(Number.isSafeInteger(v)&&v>=0);
  assert.equal(c.phase[i].reduce((a,b)=>a+b,0),c[key]);
 }
 return c.lower+c.upper;
}
async function profile() {
 const base=await buildHost({bounded:true,fastInt64:0,firFlashWord:true,sanitize:true,asmBandsModel:'pvq-row-word'});
 const out=path.join(root,'.build/opus-pvq-endpoint-word-profile');fs.mkdirSync(out,{recursive:true});
 const model=require('./pvq_row_word.cjs').cModels().find(m=>m.source==='upstream/celt/bands.c');assert.ok(model);
 const observer=path.join(__dirname,'pvq_endpoint_probe.inc.c');
 ...
 const result={schema:1,scope:'Host census of remaining endpoint-cost reads. Returns original byte, verifies aligned-word bounds; no target timing or live qualification.',recipe_sha256_lf:sourceHash(__filename),observer_sha256_lf:sourceHash(observer),parent_recipe_sha256_lf:sourceHash(path.join(__dirname,'pvq_row_word.cjs')),generated_model_sha256_lf:sourceHash(source),compiler:base.compiler,flags:base.flags,cases:[]};
 for(const f of spec){
  ...
  const probe=JSON.parse(execute('env',['ASAN_OPTIONS=detect_leaks=0','YORADIO_ENDPOINT_COUNTS='+hostPath(stats),hostPath(binary),hostPath(f.file),hostPath(observed)]));
...
module.exports={endpointAnchor,instrument,validateCounts,profile};if(require.main===module)profile().catch(e=>{console.error(e);process.exitCode=1;});
