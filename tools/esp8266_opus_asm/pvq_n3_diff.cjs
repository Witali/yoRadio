...
const previous=require('./ebands_final.cjs'),sem=require('./pvq_n3_diff_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-n3-diff-'+t+'-v1';
...
const dependencies=['pvq_n3_diff.s','pvq_n3_diff_proof.cjs','quant_decode_proof.cjs','audit_quant_decode.cjs','ebands_final.cjs','report_layout.cjs','frozen_reloads.cjs'];
const depHashes=()=>Object.fromEntries(dependencies.map(n=>[n,sourceHash(path.join(__dirname,n))]));
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'pvq_n3_diff.s'),'utf8').replace(/\r\n/g,'\n');
...
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_n3_diff_variant,'control');assert.equal(b.post_link_pvq_n3_diff_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_n3_diff_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 save(asm,source);save(script,'pvq_n3_diff_fast = 0x4025335d;\npvq_n3_diff_flags = 0x40253354;\nSECTIONS {\n'+patches.map((p,i)=>` .text.patch${i} 0x${p.address.toString(16)} : { *(.text.patch${i}) }\n`).join('')+'}\n');
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Fixed-address '+t+' exact N3 PVQ finite-difference raw ASM benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_n3_diff_variant:t,
 ...
 const helperDisassembly=layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'pvq_n3_diff_leaf',address:sem.helper,bytes:sem.helperBytes});
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-pvq-n3-diff');fs.mkdirSync(dir,{recursive:true});
 ...
 text=text.replace(anchor,`else {
        ...
        if(p>_i && _n==3){
          ...
          do {p-=delta;delta-=4;_k--;} while(p>_i);
