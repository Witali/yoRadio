// Pointwise decoder-only propagation on the accepted PVQ baseline; not production.
// Preserve all unchanged instruction addresses, including inside quant_partition.
...
const sem=require('./partition_points_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),{inspect}=require('./report_layout.cjs');
const parent=pvq.variant('candidate'),variant=t=>'esp8266-opus-partition-points-'+t+'-v1';
const art=t=>path.join(root,'firmware/development',variant(t)),build=t=>path.join(root,'.build',variant(t)),read=pvq.read;
const names=[...new Set(['quant_partition','quant_band','quant_all_bands','celt_decode_with_ec_dred',...pvq.names])];
...
const dependencies=['partition_points_proof.cjs','partition_points.s','partition_frozen_proof.cjs','partition_decode.cjs'];
...
function findPatches(functions){const patches=sem.findPatches(functions.quant_partition).patches;disjoint(patches);return patches;}
function sourceFor(){return fs.readFileSync(path.join(__dirname,'partition_points.s'),'utf8').replace(/\r\n/g,'\n');}
function prove(functions,actual,patches){
 const pointwise=sem.prove(functions.quant_partition,actual.quant_partition,patches);
 const contract=sem.contract({functions});
 for(const n of names.slice(1))assert.deepEqual(actual[n],functions[n]);
 return{pointwise,contract};
}
function manifestPair(a,b){
 assert.equal(a.post_link_partition_points_variant,'control');assert.equal(b.post_link_partition_points_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_partition_points_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){
...
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' pointwise decoder-only quant_partition ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_partition_points_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
 ...
 console.log(JSON.stringify({passed:true,sha256:manifests.candidate.app_sha256,app_bytes:app.length,ram_delta:0,constant_branches:3,zero_loads:2,patches:patches.length}));return proof;
}
function verifyPair(){
...
// Host validates the private encode=0 invariant with an abort if violated.
// Target MOVI/J equivalence is proved on linked ASM, not by host timing.
const cModels=()=>require('./partition_decode.cjs').cModels();
