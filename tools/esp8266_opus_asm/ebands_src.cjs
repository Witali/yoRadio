...
const previous=require('./ebands_final.cjs'),sem=require('./ebands_src_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-ebands-src-'+t+'-v2';
...
const dependencies=['ebands_src_proof.cjs','ebands_src.s','ebands_final.cjs','ebands_final_proof.cjs','ebands_quant_proof.cjs','ebands_more_proof.cjs','ebands_pair_proof.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'ebands_src.s'),'utf8').replace(/\r\n/g,'\n');
function manifestPair(a,b){
 assert.equal(a.post_link_ebands_src_variant,'control');assert.equal(b.post_link_ebands_src_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_ebands_src_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' branchless signed eBands pair raw benchmark; not production',app_sha256:hash(images[t]),post_link_ebands_src_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
 ...
 const helperDisassembly=layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'ebands_src',address:sem.helper,bytes:sem.helperBytes});
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-ebands-src');fs.mkdirSync(dir,{recursive:true});
 ...
 const helper=`/* Host semantic mirror: endian-neutral signed pair, no padding read. */
static int y_ebands_src_width(const CELTMode *m,int j) {
  const unsigned off=(unsigned)j*2,shift=(off&3)*8;
  opus_uint32 low,high,v;
  celt_assert(m->nbEBands==21 && j>=0 && j<21);
  memcpy(&low,(const unsigned char*)m->eBands+(off&~3u),4);
  memcpy(&high,(const unsigned char*)m->eBands+((off+2)&~3u),4);
  v=shift ? (low>>shift)|(high<<(32-shift)) : low;
  const unsigned a=v&65535,b=v>>16;
  return (b<32768?(int)b:(int)b-65536)-(a<32768?(int)a:(int)a-65536);
}
`;
 const {once}=require('./bands.cjs');text=once(text,'int clt_compute_allocation(',helper+'int clt_compute_allocation(');
 const begin=text.indexOf('int clt_compute_allocation('),start=text.indexOf('   for (j=start;j<end;j++)',begin),end=text.indexOf('   lo = 1;',start);
 assert.ok(start>begin&&end>start);const body=text.slice(start,end),anchor='y_ebands_width(m,j)';assert.equal(body.split(anchor).length-1,3);
 // GCC common-subexpression elimination shares one pair among these uses.
 text=text.slice(0,start)+body.replaceAll(anchor,'y_ebands_src_width(m,j)')+text.slice(end);
