# Exact N=3 finite-difference PVQ search, four-step Xtensa LX106 call0 leaf.
...
pvq_n3_unroll4_site:
    addi a15, a15, -4
    l32i.n a8, a15, 0
    bgeu a2, a8, pvq_n3_unroll4_fast
...
.Llinear:
    ...
    bltu a2, a8, .Llinear
    addi.n a6, a13, -1
    j pvq_n3_unroll4_flags
    .space 40 - (. - pvq_n3_unroll4_site), 0
.Larithmetic:
    call0 pvq_n3_unroll4_leaf
...
# At group entry p=U(3,K), delta=4*(K-1). Stages1/2/3 test p before
# the next subtraction; early exits subtract the matching stage from K.
# Stage4 updates K once for the whole group. Same registers and no scratch.
.section .text.patch1,"ax",@progbits
.begin no-transform
.global pvq_n3_unroll4_leaf
pvq_n3_unroll4_leaf:
    slli a6, a12, 2
    addi a6, a6, -4
.Ldifference:
...
.Lexit3:
    addi a12, a12, -3
    j .Ldone
    .space 94 - (. - pvq_n3_unroll4_leaf), 0
.end no-transform
