// Frequency and word bounds of two a4 byte probes; no target modification.
...
function instrument(text,observer) {
 ...
 text=once(text,'i==0?cache[mid]:y_pvq_word8(m,cache+mid)','i==0?y_a4_probe(m,cache+mid,0):y_pvq_word8(m,cache+mid)');
 return once(text,'LM != -1 && b > cache[cache[0]]+12 && N>2','LM != -1 && b > y_a4_probe(m,cache+cache[0],1)+12 && N>2');
}
function validateCounts(c) {
 assert.equal(c.phase.length,4);
 for(const v of[c.first,c.upper,...c.phase])assert.ok(Number.isSafeInteger(v)&&v>=0);
 assert.equal(c.phase.reduce((a,b)=>a+b,0),c.first+c.upper);
 return c.first+c.upper;
}
async function profile() {
 const base=await buildHost({bounded:true,fastInt64:0,firFlashWord:true,sanitize:true,asmBandsModel:'pvq-byte-word'});
 const out=path.join(root,'.build/opus-pvq-a4-word-profile');fs.mkdirSync(out,{recursive:true});
 const model=require('./pvq_byte_word.cjs').cModels().find(m=>m.source==='upstream/celt/bands.c');assert.ok(model);
 const observer=path.join(__dirname,'pvq_a4_probe.inc.c');
 ...
 const result={schema:1,scope:'Host census of two a4 byte reads. Returns original byte, verifies aligned-word bounds; no target timing or live qualification.',recipe_sha256_lf:sourceHash(__filename),observer_sha256_lf:sourceHash(observer),parent_recipe_sha256_lf:sourceHash(path.join(__dirname,'pvq_byte_word.cjs')),generated_model_sha256_lf:sourceHash(source),compiler:base.compiler,flags:base.flags,cases:[]};
 for(const f of spec){
  ...
  const probe=JSON.parse(execute('env',['ASAN_OPTIONS=detect_leaks=0','YORADIO_A4_COUNTS='+hostPath(stats),hostPath(binary),hostPath(f.file),hostPath(observed)]));
...
module.exports={instrument,validateCounts,profile};if(require.main===module)profile().catch(e=>{console.error(e);process.exitCode=1;});
