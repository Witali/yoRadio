// Direct signed32 exp2_table8 access on accepted index-half ASM; not production.
...
const base=require('./frozen_reloads.cjs'),previous=require('./pvq_index_half.cjs');
const sem=require('./pvq_exp2_table32_proof.cjs');
...
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-exp2-table32-'+t+'-v1';
const art=t=>path.join(root,'firmware/development',variant(t)),build=t=>path.join(root,'.build',variant(t)),read=previous.read;
const names=[...new Set(['quant_partition','quant_band','quant_all_bands','celt_decode_with_ec_dred',...previous.names,'time_service_poll'])];
...
const dependencies=['pvq_index_word_proof.cjs','pvq_index_half_proof.cjs','pvq_exp2_table32_proof.cjs','pvq_exp2_table32.s','pvq_endpoint_word_proof.cjs','frozen_reloads.cjs','pvq_row_word_proof.cjs','pvq_a4_word_proof.cjs','pvq_byte_word_proof.cjs','bits_fifth_proof.cjs','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs','report_layout.cjs'];
...
function parentFunctions(functions){const {time_service_poll,...rest}=functions;return rest;}
function tableProof(elf,candidate=false) {
 const inherited=previous.tableProof(elf),raw=sem.readAt(elf,sem.oldTable,16);
 assert.deepEqual(sem.values.map((_,i)=>raw.readInt16LE(i*2)),sem.values);
 assert.equal(sem.readAt(elf,sem.pool,4).readUInt32LE(),candidate?sem.table:sem.oldTable,'exp2 literal pointer');
 const words=Buffer.alloc(32);sem.values.forEach((v,i)=>words.writeInt32LE(v,i*4));assert.equal(sem.table%4,0);
 if(candidate)assert.deepEqual(sem.readAt(elf,sem.table,32),words,'Signed32 table copy');
 return {...inherited,exp2_original_address:sem.oldTable,exp2_original_bytes:16,exp2_original_sha256:hash(raw),exp2_word_address:sem.table,exp2_word_bytes:32,exp2_word_sha256:hash(words),exp2_literal:sem.pool};
}
function generate(){
 ...
 const info=inspect(parent,names);assert.deepEqual(parentFunctions(info.functions),prior.proof.actual_functions);
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' direct signed32 exp2_table8 ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_exp2_table32_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
...
function verifyPair(){
 ...
 assert.deepEqual(prove(proof.functions,proof.actual_functions,proof.patches),proof.semantic);assert.deepEqual(sem.storageBytesProof(a),proof.original_storage);assert.deepEqual(tableProof(a),proof.table);assert.deepEqual(tableProof(b,true),proof.table);
 assert.deepEqual(sem.literalReadersProof(a,proof.functions.time_service_poll),proof.literal_readers);assert.deepEqual(sem.literalReadersProof(b,proof.actual_functions.time_service_poll),proof.literal_readers);
 ...
 assert.deepEqual(parentFunctions(proof.functions),prior.proof.actual_functions);
 return{proof,manifests};
}
// Mirror only the quant_partition/non-stereo compute_qn clone; C fallback unchanged.
// Static words are flash in the target overlay, not an extra DRAM table.
function cModels(){
 const dir=path.join(root,'.build/opus-bands-pvq-exp2-table32');fs.mkdirSync(dir,{recursive:true});
 const {once}=require('./bands.cjs');
 return previous.cModels().map(model=>{
  let text=fs.readFileSync(model.file,'utf8').replace(/\r\n/g,'\n');
  const helper=`static const opus_int32 y_pvq_exp2_table32[8] = {
  16384,17866,19483,21247,23170,25267,27554,30048
};
`;
  text=once(text,'static int compute_qn(',helper+'static int compute_qn(');
  text=once(text,'qn = exp2_table8[qb&0x7]>>(14-(qb>>BITRES));',
   'qn = (stereo ? exp2_table8[qb&0x7] : y_pvq_exp2_table32[qb&0x7])>>(14-(qb>>BITRES));');
