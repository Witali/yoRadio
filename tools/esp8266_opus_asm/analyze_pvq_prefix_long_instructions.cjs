...
const {root,hash,sourceHash}=require('./export.cjs'),f=require('./pvq_prefix_long.cjs'),sem=require('./pvq_prefix_long_proof.cjs');
function analyze(){
 ...
 const prefix=sem.prefix(p.table),extra=new Map([[sem.dataAddress,sem.prefixAddress-96]]);
 ...
 scope:'Instruction and load min/max over all residual indices consistent with each recorded answer. Includes pointer-literal and packed-prefix word loads separately. Not cycles/cache timing; actual residual indices are not in these traces.',cases:[]};
 for(const c of census.cases){
   ...
   const longGuard={hypothetical:false,condition:'K-N>=8 && index<U(N,K-4)',prefix_calls:0,extra_U_reads:0};
   for(const[n,k,t]of rows){
     ...
     for(const index of points){
       ...
       assert.equal(literal,k-n>=8&&k-t>=5?1:0);assert.equal(metadata,literal);
     ...
     if(k-n>=8){
       if(k-t>=5){sums.literal_reads++;sums.prefix_word_reads++;sums.helper_calls++;}
   ...
   result.cases.push({name:c.name,searches:rows.length,...sums,long_guard:longGuard,
