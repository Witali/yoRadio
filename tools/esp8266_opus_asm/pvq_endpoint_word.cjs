// Exact endpoint word access on accepted row-word ASM; not production.
...
const base=require('./frozen_reloads.cjs'),previous=require('./pvq_row_word.cjs');
const sem=require('./pvq_endpoint_word_proof.cjs');
...
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-endpoint-word-'+t+'-v1';
...
const dependencies=['pvq_endpoint_word_proof.cjs','pvq_endpoint_word.s','pvq_row_word_proof.cjs','pvq_a4_word_proof.cjs','pvq_byte_word_proof.cjs','bits_fifth_proof.cjs','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs','report_layout.cjs'];
...
function sourceFor(){return fs.readFileSync(path.join(__dirname,'pvq_endpoint_word.s'),'utf8').replace(/\r\n/g,'\n');}
...
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_endpoint_word_variant,'control');assert.equal(b.post_link_pvq_endpoint_word_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_endpoint_word_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){
...
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' word-load PVQ byte helper ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_endpoint_word_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
...
function cModels(){
 const dir=path.join(root,'.build/opus-bands-pvq-endpoint-word');fs.mkdirSync(dir,{recursive:true});
 const {once}=require('./bands.cjs');
 return previous.cModels().map(model=>{
  let text=fs.readFileSync(model.file,'utf8').replace(/\r\n/g,'\n');
  text=once(text,' const int lower=lo==0?-1:(int)cache[lo],upper=(int)cache[hi];',
   ' const int lower=lo==0?-1:y_pvq_word8(m,cache+lo),upper=y_pvq_word8(m,cache+hi);');
