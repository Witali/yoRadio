// Fresh raw A/B/A for the fixed-address local scalar experiment; retain every attempt.
const fs=require('node:fs'),path=require('node:path'),assert=require('node:assert/strict');
const {root}=require('./export.cjs'),f=require('./quant_locals.cjs');
...
const dest=f.art('candidate'),experiment=path.join(root,'.build/opus-quant-locals-board-20260918');
function report(){
