...
const {root}=require('./export.cjs'),f=require('./ebands_u16_aligned.cjs');
...
const dest=f.art('candidate'),experiment=path.join(root,'.build/opus-ebands-u16-aligned-board-20260917'),read=f.read;
function report(){
 ...
 for(const name of['before','candidate','after']){
  ...
  const rows=archive(path.join(experiment,name),path.join(dest,name));assert.equal(rows.length,10);for(const r of rows)validateRun(r.report,fixtures,ota.after.app_address);groups[name]=rows;
 ...
 const r={pair,initial,repeated,selection:{initial:selectHighBitrate(initial.cases),repeated:selectHighBitrate(repeated.cases)},inputs:Object.fromEntries(Object.entries(groups).map(([k,rs])=>[k,rs.map(({report,...x})=>x)])),scope:'All30 independent A/B/A attempts at15s polling. RAM packets, raw CPU; no output or profilers. Not live qualification.',current_cpu_target:evaluateRawCpuTarget(initial.cases,75)};
 fs.writeFileSync(path.join(dest,'comparison.json'),JSON.stringify(r,null,2)+'\n');console.table(initial.cases.map((c,i)=>({name:c.name,A:c.reference.task_budget_percent.median,B:c.candidate.task_budget_percent.median,A2:repeated.cases[i].reference.task_budget_percent.median})));console.log(JSON.stringify(r.selection));return r;
}
