// Frozen-layout ASM experiment over accepted ebands-more; not production.
...
const previous=require('./ebands_more.cjs'),sem=require('./allocation_byte_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-allocation-byte-'+t+'-v1';
...
const dependencies=['allocation_byte_proof.cjs','allocation_byte.s','ebands_more.cjs','ebands_more_proof.cjs','ebands_pair_proof.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'allocation_byte.s'),'utf8').replace(/\r\n/g,'\n');
function manifestPair(a,b){
 assert.equal(a.post_link_allocation_byte_variant,'control');assert.equal(b.post_link_allocation_byte_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_allocation_byte_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){assert.equal(m.post_link_ebands_more_variant,'candidate');assert.equal(m.opus_backend,'bands-tell-inline-asm');assert.equal(m.cpu_mhz,160);assert.equal(m.flash,'QIO40');assert.equal(m.opus_benchmark,true);assert.equal(m.opus_benchmark_output,false);assert.equal(m.opus_function_profile,false);assert.equal(m.opus_profile_stage,0);}
}
function generate(){
 ...
 const helperDisassembly=sem.specs.map((s,i)=>layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'allocation_byte_'+i,address:s.helper,bytes:sem.helperBytes}));
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-allocation-byte');fs.mkdirSync(dir,{recursive:true});
 ...
 const anchor='m->allocVectors[mid*len+j]';assert.equal(text.split(anchor).length,2);
 const helper=`static int y_allocation_byte(const CELTMode *m,int index) {\n  opus_uint32 word=0;\n  celt_assert(m->nbEBands==21 && m->nbAllocVectors==11 && index>=0 && index<231);\n  unsigned offset=(unsigned)index&~3u;\n  unsigned count=231-offset; if(count>4) count=4;\n  /* C model avoids a C object overread; target's fourth padding byte is\n     physically mapped flash and symbolically proven irrelevant. */\n  memcpy(&word,m->allocVectors+offset,count);\n  return (word>>((index&3)*8))&255;\n}\n`;
 const {once}=require('./bands.cjs');text=once(text,'int clt_compute_allocation(',helper+'int clt_compute_allocation(');
 text=once(text,anchor,'y_allocation_byte(m,mid*len+j)');
