// Frequency of the remaining adjustment-loop byte read; no target change/timing.
...
const anchor='         curr_bits = pulses2bits(m, i, LM, q);';
function instrument(text,observer){text=once(text,'static unsigned quant_partition(',observer+'\nstatic unsigned quant_partition(');return once(text,anchor,'         curr_bits = y_adjust_probe(m,i,LM,q);');}
function validateCounts(c){
 assert.equal(c.phase.length,4);for(const n of[c.calls,c.loads,c.zero,...c.phase])assert.ok(Number.isSafeInteger(n)&&n>=0);
 assert.equal(c.calls,c.loads+c.zero);assert.equal(c.phase.reduce((a,b)=>a+b,0),c.loads);return c.loads;
}
async function profile(){
 const base=await buildHost({bounded:true,fastInt64:0,firFlashWord:true,sanitize:true,asmBandsModel:'pvq-index-half'});
 const out=path.join(root,'.build/opus-pvq-adjustment-profile');fs.mkdirSync(out,{recursive:true});
 const model=require('./pvq_index_half.cjs').cModels().find(m=>m.source==='upstream/celt/bands.c');assert.ok(model);
 const observer=path.join(__dirname,'pvq_adjust_probe.inc.c');
 ...
 for(const f of spec){
  ...
  const probe=JSON.parse(execute('env',['ASAN_OPTIONS=detect_leaks=0','YORADIO_ADJUST_COUNTS='+hostPath(stats),hostPath(binary),hostPath(f.file),hostPath(observed)]));
