// Opt-in fixed-address ASM experiment over accepted eBands-final; not production.
...
const previous=require('./ebands_final.cjs'),sem=require('./quant_locals_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-quant-locals-'+t+'-v1';
...
const dependencies=['quant_locals_proof.cjs','quant_decode_proof.cjs','audit_quant_decode.cjs','ebands_final.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
function manifestPair(a,b){
 assert.equal(a.post_link_quant_locals_variant,'control');assert.equal(b.post_link_quant_locals_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_quant_locals_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 const contract=sem.contract(info,inputFile),patches=sem.findPatches(info.functions.quant_all_bands),source=sem.sourceFor(patches);
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Same-address '+t+' local scalar load quant_all_bands raw ASM benchmark; not production',app_sha256:hash(images[t]),post_link_quant_locals_variant:t,
 ...
 for(const n of names.filter(n=>n!=='quant_all_bands'))assert.deepEqual(actual.functions[n],info.functions[n]);
 const semantic=sem.prove(info.functions.quant_all_bands,actual.functions.quant_all_bands,patches);
 const proof={schema:1,parent,recipe_sha256_lf:sourceHash(__filename),dependencies:depHashes(),parent_elf_sha256:hash(a),candidate_elf_sha256:hash(b),
  patches,contract,semantic,static_ram_delta:0,stack_delta:0,app_bytes:app.length,sections:actual.sections,functions:info.functions,actual_functions:actual.functions,
...
function verifyPair(){
 ...
 assert.deepEqual(sem.prove(p.functions.quant_all_bands,p.actual_functions.quant_all_bands,p.patches),p.semantic);
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-quant-locals');fs.mkdirSync(dir,{recursive:true});
 ...
 const anchor='static OPUS_INLINE int band_encode(const struct band_ctx *ctx)';assert.equal(text.split(anchor).length-1,1);
 text=text.replace(anchor,'static OPUS_INLINE int y_quant_flag(int value, int expected)\n{\n   if (value != expected) abort();\n   return expected;\n}\n'+anchor);
 assert.equal(text.split('return ctx->encode;').length-1,1);
 text=text.replace('return ctx->encode;','return y_quant_flag(ctx->encode, 0);');
 const count=text.split('ctx->resynth').length-1;assert.ok(count>0);
 text=text.replaceAll('ctx->resynth','y_quant_flag(ctx->resynth, 1)');
 // Host checks the origins of spilled constants, not Xtensa register timing.
 // lowband_offset is only initially zero; later updates/reads stay dynamic.
 for(const [from,to]of[['int resynth = 1;','int resynth = y_quant_flag(1, 1);'],
  ['int resynth = !encode || theta_rdo;','int resynth = y_quant_flag(!encode || theta_rdo, 1);'],
  ['lowband_offset = 0;','lowband_offset = y_quant_flag(0, 0);']]){
  assert.equal(text.split(from).length-1,1);text=text.replace(from,to);
 }
