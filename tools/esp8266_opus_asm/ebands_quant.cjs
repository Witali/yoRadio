// Frozen-layout experiment: reuse accepted eBands leaf in quant_all_bands.
...
const previous=require('./ebands_final.cjs'),sem=require('./ebands_quant_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-ebands-quant-'+t+'-v1';
...
const dependencies=['ebands_quant_proof.cjs','ebands_quant.s','ebands_final.cjs','ebands_final_proof.cjs','ebands_more_proof.cjs','ebands_pair_proof.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'ebands_quant.s'),'utf8').replace(/\r\n/g,'\n');
function manifestPair(a,b){
 assert.equal(a.post_link_ebands_quant_variant,'control');assert.equal(b.post_link_ebands_quant_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_ebands_quant_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 const info=inspect(parent);assert.deepEqual(info.functions,inherited.proof.actual_functions);
 const patches=sem.findPatches(info.functions),source=sourceFor(),leaf=sem.sharedLeaf(a);
 ...
 save(asm,source);save(script,sem.definitions()+'\nSECTIONS { .text.patch0 0x'+sem.site.toString(16)+' : { *(.text.patch0) } }\n');
 ...
 const b=base.patchElf(a,patches),images={},manifests={},logs={};assert.deepEqual(sem.sharedLeaf(b),leaf);
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' eBands quant pair raw benchmark; not production',app_sha256:hash(images[t]),post_link_ebands_quant_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
 ...
 const helperDisassembly=layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'ebands_init',address:sem.helper,bytes:sem.helperBytes});assert.equal(helperDisassembly,leaf.disassembly);
 ...
 const proof={schema:1,parent,recipe_sha256_lf:sourceHash(__filename),dependencies:depHashes(),parent_elf_sha256:hash(a),candidate_elf_sha256:hash(b),shared_leaf:leaf,patches,semantic,table,helperDisassembly,static_ram_delta:0,stack_delta:0,app_bytes:app.length,sections:actual.sections,functions:info.functions,actual_functions:actual.functions,imageProof:base.compareApps(images.control,images.candidate,patches),packaging:{esptool_sha256:hash(fs.readFileSync(esptool)),logs}};
...
function verifyPair(){
 ...
 assert.deepEqual(sem.prove(p.functions,p.actual_functions,p.helperDisassembly),p.semantic);assert.deepEqual(sem.tableProof(a),p.table);assert.deepEqual(sem.tableProof(b),p.table);assert.deepEqual(sem.sharedLeaf(a),p.shared_leaf);assert.deepEqual(sem.sharedLeaf(b),p.shared_leaf);
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-ebands-quant');fs.mkdirSync(dir,{recursive:true});
 const source='upstream/celt/bands.c',old=models.find(m=>m.source===source);assert.ok(old);
 let text=fs.readFileSync(old.file,'utf8').replace(/\r\n/g,'\n');const {once}=require('./bands.cjs');
 const helper=`/* Host-only semantic mirror of aligned word reads in accepted leaf. */
#include <string.h>
static void y_quant_ebands_pair(const opus_int16 *p,int *lo,int *hi) {
 uintptr_t addr=(uintptr_t)p; opus_uint32 w;
 memcpy(&w,(const void *)(addr&~(uintptr_t)3),4);
 if(addr&2) { *lo=(opus_int16)(w>>16); memcpy(&w,(const void *)(addr+2),4); *hi=(opus_int16)w; }
 else { *lo=(opus_int16)w; *hi=(opus_int16)(w>>16); }
}
`;
 text=once(text,'void quant_all_bands(',helper+'\nvoid quant_all_bands(');
 text=once(text,'      X = X_+M*eBands[i];','      int y_lo,y_hi; y_quant_ebands_pair(eBands+i,&y_lo,&y_hi);\n      X = X_+M*y_lo;');
 text=once(text,'         Y = Y_+M*eBands[i];','         Y = Y_+M*y_lo;');
 text=once(text,'      N = M*eBands[i+1]-M*eBands[i];','      N = M*y_hi-M*y_lo;');
 const file=path.join(dir,'bands.model.c');fs.writeFileSync(file,text);return models.map(m=>m.source===source?{source,file}:m);
}
