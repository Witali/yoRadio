...
function checkHelper(text){
 ...
 const expected=['slli a6, a12, 2','addi a6, a6, -4','sub a8, a8, a6','addi a6, a6, -4','addi a12, a12, -1',null,'addi a6, a13, -1','ret'];
 assert.equal(rs.length,expected.length);assert.equal(rs[0].address,helper);
 expected[5]='bltu a2, a8, '+rs[2].address.toString(16);assert.deepEqual(ops,expected);
...
function prove(before,after,helperText,t){
 ...
 return{cases,arithmetic_cases:arithmeticCases,fast_cases:fastCases,helper_instructions:h.length,linear_reads:linearReads,candidate_reads:binaryReads,max_candidate_instructions:maxSteps,
  ...
  invariant:'Only N=3 after failed initial U(K) probe. Delta=4*(K-1); p-=delta, delta-=4, K-- preserves p=U(3,K) exactly. K>=3 since index>=U(3,3)=13. Full stored domain K<=175,p<=60901,delta<=696: no overflow/underflow. Other N and initial fast return unchanged. No loads in the helper, tables or bitrate caps.',
