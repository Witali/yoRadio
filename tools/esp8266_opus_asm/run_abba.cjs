...
function checkArtifacts(aDir,bDir,fixtures){
...
async function run({reference,candidate,fixtures,directory,base='http://192.168.100.6',cycles=5}){
  const artifacts=checkArtifacts(reference,candidate,fixtures),order=plan(cycles);
  // New directory only: never overwrite evidence or infer completion from a lock.
  fs.mkdirSync(directory,{recursive:false});
  const report={date:new Date().toISOString(),base,artifacts,order,events:[],complete:false};
  ...
  try{
    ...
    for(const [index,backend] of order.entries()){
      ...
      await child('tools/esp8266_opus_profile/run_board.cjs',['--base',base,'--fixtures',fixtures,'--output',path.join(directory,file)],'run-'+label+'.log');
    ...
    result.artifacts=artifacts;result.order=order;
...
if(require.main===module){
  ...
  run({reference:value('--reference'),candidate:value('--candidate'),fixtures:value('--fixtures'),directory:value('--directory')}).catch(e=>{console.error(e);process.exitCode=1});
}
