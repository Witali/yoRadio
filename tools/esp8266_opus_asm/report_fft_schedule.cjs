...
const {root,hash,sourceHash}=require('./export.cjs'),f=require('./fft_schedule.cjs'),base=require('./frozen_reloads.cjs');
...
const dest=f.art('candidate'),experiment=path.join(root,'.build/opus-fft-schedule-board'),read=f.read;
function verifyPair(){
 const proof=read(path.join(dest,'preflight.json'));
 assert.equal(proof.recipe_sha256_lf,sourceHash(path.join(__dirname,'fft_schedule.cjs')));assert.equal(proof.asm_sha256_lf,sourceHash(f.sourceFile));assert.equal(proof.asm_sha256_lf,sourceHash(path.join(dest,'patches.s')));
 ...
 const object=fs.readFileSync(path.join(dest,'patches.o')),s=sections(object).find(s=>s.name==='.text.patch0');assert.equal(s.bytes,19);assert.equal(object.subarray(s.offset,s.offset+s.bytes).toString('hex'),after_hex);
 ...
 assert.deepEqual(outside(old),outside(now));assert.deepEqual(inside(now).map(r=>r.bytes),[3,3,2,3,3,3,2]);assert.deepEqual(inside(now).map(r=>r.text),f.replacement);assert.deepEqual(f.prove(inside(old).map(r=>r.text),inside(now).map(r=>r.text)),proof.symbolic);
 assert.deepEqual(f.numericProof(f.original,inside(now).map(r=>r.text)),proof.numeric);
