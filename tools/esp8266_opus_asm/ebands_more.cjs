// Frozen-layout ASM experiment over accepted ebands-pair; not production.
...
const previous=require('./ebands_pair.cjs'),sem=require('./ebands_more_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-ebands-more-'+t+'-v1';
...
const dependencies=['ebands_more_proof.cjs','ebands_more.s','ebands_pair.cjs','ebands_pair_proof.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'ebands_more.s'),'utf8').replace(/\r\n/g,'\n');
function manifestPair(a,b){
 assert.equal(a.post_link_ebands_more_variant,'control');assert.equal(b.post_link_ebands_more_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_ebands_more_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){assert.equal(m.post_link_ebands_pair_variant,'candidate');assert.equal(m.opus_backend,'bands-tell-inline-asm');assert.equal(m.cpu_mhz,160);assert.equal(m.flash,'QIO40');assert.equal(m.opus_benchmark,true);assert.equal(m.opus_benchmark_output,false);assert.equal(m.opus_function_profile,false);assert.equal(m.opus_profile_stage,0);}
}
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' eBands pair raw benchmark; not production',app_sha256:hash(images[t]),post_link_ebands_more_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
 ...
 const helperDisassembly=sem.specs.map((s,i)=>layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'ebands_more_'+i,address:s.helper,bytes:sem.helperBytes}));
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-ebands-more');fs.mkdirSync(dir,{recursive:true});
 ...
 // Only the initialization loop and final allocation-vector interpolation.
 // The previous binary-search pair remains inherited, other inlined loops
 // in interp_bits2pulses remain unchanged.
 const begin=text.indexOf('int clt_compute_allocation(');assert.ok(begin>0);
 const prefix=text.slice(0,begin),body=text.slice(begin),anchor='m->eBands[j+1]-m->eBands[j]';
 assert.equal(body.split(anchor).length-1,4);
 text=prefix+body.replaceAll(anchor,'y_ebands_width(m,j)');
 const file=path.join(dir,'rate.model.c');fs.writeFileSync(file,text);
 return models.map(m=>m.source===source?{source,file}:m);
}
