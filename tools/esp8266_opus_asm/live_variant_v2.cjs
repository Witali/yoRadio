...
const defaultBaseVariant='esp8266-opus-live-asm-input2k-base-20260917';
const defaultVariant='esp8266-opus-live-asm-input2k-20260917';
...
const {literalReader}=require('./literal_readers.cjs');
function generate(options={}){
 const baseVariant=options.baseVariant||defaultBaseVariant,variant=options.variant||defaultVariant;
 assert.ok(/^[a-zA-Z0-9_-]+$/.test(baseVariant)&&/^[a-zA-Z0-9_-]+$/.test(variant)&&baseVariant!==variant,'Distinct safe variant names required');
...
module.exports={names,chain,mapping,generate};
if(require.main===module)generate({baseVariant:process.argv[2],variant:process.argv[3]});
