...
const previous=require('./ebands_final.cjs'),sem=require('./quant_decode_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-quant-decode-'+t+'-v2';
...
const dependencies=['quant_decode_proof.cjs','audit_quant_decode.cjs','ebands_final.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
function manifestPair(a,b){
 assert.equal(a.post_link_quant_decode_variant,'control');assert.equal(b.post_link_quant_decode_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_quant_decode_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 const contract=sem.contract(info,inputFile),compiled=sem.compile(info.functions.quant_all_bands);
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' decoder-only quant_all_bands raw benchmark; not production',app_sha256:hash(images[t]),post_link_quant_decode_variant:t,
 ...
 const semantic=sem.prove(info.functions.quant_all_bands,actual.functions.quant_all_bands);
...
function verifyPair(){
 ...
 assert.deepEqual(sem.prove(p.functions.quant_all_bands,p.actual_functions.quant_all_bands),p.semantic);
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-quant-decode');fs.mkdirSync(dir,{recursive:true});
 const source='upstream/celt/bands.c',old=models.find(m=>m.source===source);assert.ok(old);
 const text='#undef YORADIO_OPUS_CELT_DECODE_ONLY\n#define YORADIO_OPUS_CELT_DECODE_ONLY 1\n'+fs.readFileSync(old.file,'utf8').replace(/\r\n/g,'\n');
