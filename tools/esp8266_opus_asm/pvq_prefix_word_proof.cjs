...
const dataAddress=0x40248cb4,prefixAddress=dataAddress+4,prefixBytes=1408,dataBytes=prefixBytes+4,algBytes=0x6fc;
...
function auditSite(fn,candidate=false){
 ...
 assert.deepEqual(inside(fn).map(clean),candidate?['addi a15, a15, -4','sub a8, a12, a13','bgei a8, 8, 40253351','l32i a8, a15, 0','bgeu a2, a8, 4025335d','addi a6, a15, -4','l32i a8, a6, 0','addi a6, a6, -4','addi a12, a12, -1','bltu a2, a8, 4025333a','addi a6, a13, -1','j 40253354','call0 40251024']:oldOps);
...
function storage(info,elf,bytes){
 ...
 invariant:'The sole linked alg_quant call is in the proven encode=0 dead set. No allocated address-taken reference or external jump enters its function. Only its first1412 bytes are replaced by a literal and flash data; valid decoder flow cannot execute them.'}};
...
function prefix(t){
 ...
 for(const row of rows)for(let z=0;z<32;z++){
   ...
   bytes.writeUInt32LE(k,((row.n-3)*32+z)*4);
 }
 return bytes;
}
function prefixProof(bytes,t){
 const p=prefix(t);assert.equal(readAt(bytes,dataAddress,4).readUInt32LE(),prefixAddress-384);
...
function prove(before,after,helperText,t){
 ...
 const packed=prefix(t),extra=new Map([[dataAddress,prefixAddress-384]]);
 ...
 for(const {n,address:B,values}of t.rows)for(let K=n;K<n+values.length-1;K++){
   ...
   for(let tail=n;tail<=K;tail++){
     ...
     for(const index of points)for(const seed of[0,0xffffffff]){
       ...
       if(K-n>=8){const phase=Math.clz32(index)&3;if(!witnesses.has(phase))witnesses.set(phase,{r,memory});}
     }
   }
 }
 ...
 return{cases,extra_sar_cases:extraSarCases,helper_instructions_covered:h.length,linear_reads:linearReads,prefix_reads_including_literal:prefixReads,max_candidate_instructions:maxSteps,
 stack_bytes:48,stack_delta:0,static_ram_delta:0,stores:0,sar_preserved:true,
 invariant:'For e=floor(log2(index)), table[N,e] is the greatest stored j with U(N,j)<=2^(e+1)-1. It is >= the true answer. MIN with original K preserves this upper bound, then descending exact search terminates at the unchanged index>=U(N,N) guard. Every prefix read is aligned32-bit and contains one complete bound. No SAR instructions. Short K-N<8 searches stay linear.',
 scope:'Every U interval at boundaries/midpoint AND both sides of every CLZ boundary, two register seeds; all64 SAR states for four CLZ classes. Actual linked instructions, not CPU timings. a0/a3 dead at join; all other GPRs/SAR match. Prefix table may contain conservative extra row-end bounds, clamped by original K.'};
}
