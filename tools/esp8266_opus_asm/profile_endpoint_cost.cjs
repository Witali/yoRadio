// Frequency/correctness of reusing both endpoint byte costs. Host only.
...
function cached(cache,bits) {
 let lo=0,hi=cache[0];bits--;
 for(let i=0;i<4;i++){const m=(lo+hi+1)>>1;if(cache[m]>=bits)hi=m;else lo=m;}
 if(hi-lo>1){const m=(lo+hi+1)>>1;if(cache[m]>=bits)hi=m;else lo=m;
  if(hi-lo>1){const n=(lo+hi+1)>>1;if(cache[n]>=bits)hi=n;else lo=n;}}
 const lower=lo===0?-1:cache[lo],upper=cache[hi],useLower=bits-lower<=upper-bits,q=useLower?lo:hi;
 return {q,cost:q===0?0:(useLower?lower:upper)+1,useLower,lo,hi};
}
function exhaustive() {
 const {bits,offsets}=require('./pulse_lookup.cjs').tables(),{reference}=require('./pulse_inverse.cjs');
 const r={scope:'Uniform standard-table budgets, NOT stream frequency or CPU evidence',cases:0,zero:0,lower:0,upper:0,same:0};
 for(const off of offsets)for(let b=-64;b<=16383;b++){
  const cache=bits.slice(off),v=cached(cache,b);assert.equal(v.q,reference(cache,b));assert.equal(v.cost,v.q===0?0:cache[v.q]+1);
  r.cases++;r[v.q===0?'zero':v.useLower?'lower':'upper']++;if(v.lo===v.hi)r.same++;
 }return r;
}
async function profile() {
 const base=await buildHost({bounded:true,fastInt64:0,firFlashWord:true,sanitize:true,asmBandsModel:'tell-inline'});
 const out=path.join(root,'.build/opus-endpoint-cost-profile');fs.mkdirSync(out,{recursive:true});
 ...
 const observer=path.join(__dirname,'endpoint_cost_probe.inc.c');
 text=once(text,'#include "rate.h"\n','#include "rate.h"\n'+fs.readFileSync(observer,'utf8')+'\n');
 text=once(text,'      q = bits2pulses(m, i, LM, b);','      q = y_endpoint_probe(m, i, LM, b);');
 ...
 for(const f of spec){
  ...
  const counts=JSON.parse(fs.readFileSync(stats));assert.equal(counts.zero+counts.lower+counts.upper,counts.calls);assert.ok(counts.same<=counts.calls);
  const seconds=probe.samples/48000,entry={name:f.name,fixture_sha256:hash(fs.readFileSync(f.file)),reference,probe,pcm,counts,seconds,calls_per_audio_second:counts.calls/seconds,cached_percent:counts.calls?100*(counts.lower+counts.upper)/counts.calls:null};
  result.cases.push(entry);console.log(f.name,JSON.stringify({counts,percent:entry.cached_percent}));
 }
 result.passed=true;fs.writeFileSync(path.join(out,'summary.json'),JSON.stringify(result,null,2)+'\n');return result;
}
module.exports={cached,exhaustive,profile};if(require.main===module)profile().catch(e=>{console.error(e);process.exitCode=1;});
