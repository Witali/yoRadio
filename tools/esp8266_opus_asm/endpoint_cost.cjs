// Cached PVQ endpoint cost on the accepted baseline; not production.
...
const base=require('./frozen_reloads.cjs'),previous=require('./bits_fourth.cjs');
const sem=require('./endpoint_cost_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),{inspect}=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-endpoint-cost-'+t+'-v1';
...
const dependencies=['endpoint_cost_proof.cjs','endpoint_cost.s','bits_fifth_proof.cjs','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs'];
...
function sourceFor(){return fs.readFileSync(path.join(__dirname,'endpoint_cost.s'),'utf8').replace(/\r\n/g,'\n');}
function prove(functions,actual,patches){
 const search=sem.prove(functions.quant_partition,actual.quant_partition);
 const contract={scope:'Unchanged caller chain and pure-register search replacement; inherit the independently verified bits-fourth private encode=0 contract.'};
 for(const n of names.slice(1))assert.deepEqual(actual[n],functions[n]);
 return{search,contract};
}
function manifestPair(a,b){
 assert.equal(a.post_link_endpoint_cost_variant,'control');assert.equal(b.post_link_endpoint_cost_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_endpoint_cost_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){
  assert.equal(m.post_link_bits_fourth_variant,'candidate');assert.equal(m.post_link_bits_fifth_variant,'candidate');assert.equal(m.post_link_pvq_row_loop_variant,'candidate');assert.equal(m.post_link_post_pair_variant,'candidate');
...
function cModels(){
 const dir=path.join(root,'.build/opus-bands-endpoint-cost');fs.mkdirSync(dir,{recursive:true});
 ...
 return require('./tell_inline.cjs').cModels().map(model=>{
  ...
  text=once(text,'#include "rate.h"\n','#include "rate.h"\n'+[
   'static int y_endpoint_cost(const CELTMode *m,int band,int LM,int bits,int *cost) {',
   ...
   ' const int use_lower=bits-lower<=upper-bits,q=use_lower?lo:hi;',
   ' *cost=q==0?0:(use_lower?lower:upper)+1;return q;',
   '}'].join('\n')+'\n');
  text=once(text,'      q = bits2pulses(m, i, LM, b);\n      curr_bits = pulses2bits(m, i, LM, q);','      q = y_endpoint_cost(m, i, LM, b, &curr_bits);');
