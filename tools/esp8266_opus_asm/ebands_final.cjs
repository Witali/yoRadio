// Frozen-layout ASM experiment over accepted allocation-byte; not production.
...
const previous=require('./allocation_byte.cjs'),sem=require('./ebands_final_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-ebands-final-'+t+'-v2';
...
const dependencies=['ebands_final_proof.cjs','ebands_final.s','allocation_byte.cjs','ebands_more_proof.cjs','ebands_pair_proof.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'ebands_final.s'),'utf8').replace(/\r\n/g,'\n');
function manifestPair(a,b){
 assert.equal(a.post_link_ebands_final_variant,'control');assert.equal(b.post_link_ebands_final_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_ebands_final_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){assert.equal(m.post_link_allocation_byte_variant,'candidate');assert.equal(m.opus_backend,'bands-tell-inline-asm');assert.equal(m.cpu_mhz,160);assert.equal(m.flash,'QIO40');assert.equal(m.opus_benchmark,true);assert.equal(m.opus_benchmark_output,false);assert.equal(m.opus_function_profile,false);assert.equal(m.opus_profile_stage,0);}
}
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' final eBands pairs raw benchmark; not production',app_sha256:hash(images[t]),post_link_ebands_final_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
 ...
 const helperDisassembly=sem.specs.map((s,i)=>layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'ebands_final_'+i,address:s.helper,bytes:s.span}));
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-ebands-final');fs.mkdirSync(dir,{recursive:true});
 ...
 // Forward declaration for the existing word-width helper, located later.
 const {once}=require('./bands.cjs');
 text=once(text,'static OPUS_INLINE int interp_bits2pulses(', 'static int y_ebands_width(const CELTMode *m,int j);\nstatic OPUS_INLINE int interp_bits2pulses(');
 const begin=text.indexOf('static OPUS_INLINE int interp_bits2pulses('),end=text.indexOf('#include <string.h>',begin);assert.ok(begin>0&&end>begin);
 const body=text.slice(begin,end),anchor='m->eBands[j+1]-m->eBands[j]';assert.equal(body.split(anchor).length-1,3);
 text=text.slice(0,begin)+body.replaceAll(anchor,'y_ebands_width(m,j)')+text.slice(end);
