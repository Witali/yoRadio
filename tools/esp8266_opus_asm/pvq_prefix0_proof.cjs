...
function prove(before,after,helperText,t){
 ...
 invariant:'For e=floor(log2(index)), table[N,e] is the greatest stored j with U(N,j)<=2^(e+1)-1. It is >= the true answer. MIN with original K preserves this upper bound, then descending exact search terminates at the unchanged index>=U(N,N) guard. Every prefix read is aligned32-bit; extraction selects one of four bytes and restores SAR before any data branch. Short K-N<8 searches stay linear.',
