// Exact signed index word access on accepted endpoint-word ASM; not production.
...
const base=require('./frozen_reloads.cjs'),previous=require('./pvq_endpoint_word.cjs');
const sem=require('./pvq_index_word_proof.cjs');
...
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-index-word-'+t+'-v2';
...
const dependencies=['pvq_index_word_proof.cjs','pvq_index_word.s','pvq_endpoint_word_proof.cjs','frozen_reloads.cjs','pvq_row_word_proof.cjs','pvq_a4_word_proof.cjs','pvq_byte_word_proof.cjs','bits_fifth_proof.cjs','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs','report_layout.cjs'];
...
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' signed-index word helper ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_index_word_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
...
function cModels(){
 const dir=path.join(root,'.build/opus-bands-pvq-index-word');fs.mkdirSync(dir,{recursive:true});
...
static int y_pvq_index_word(const CELTMode *m,int pos) {
  opus_uint32 word=0;
  ...
  const unsigned value=(word>>((offset&3)*8))&65535;
...
`;
  ...
  text=once(text,'   cache = m->cache.bits + m->cache.index[(LM+1)*m->nbEBands+i];',
   '   cache = m->cache.bits + y_pvq_index_word(m,(LM+1)*m->nbEBands+i);');
