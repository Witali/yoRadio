...
const {root}=require('./export.cjs'),f=require('./pvq_qn_table.cjs');
...
const dest=f.art('candidate'),experiment=path.join(root,'.build/opus-qn-table-raw15s-board-20260917'),read=f.read;
function report(){
 ...
 for(const[name,dir]of[['before','qualified/before'],['candidate','qualified/candidate'],['after','qualified/after']]){
 ...
 const result={pair,initial,repeated,selection:{initial:selectHighBitrate(initial.cases),repeated:selectHighBitrate(repeated.cases)},
  ...
  scope:'All30 independent frozen-layout v+1 qn table A/B/A raw attempts retained. Packets in RAM; no output or function/stage profiler. Not live qualification.',
