// Frozen exact v+1 lookup over accepted exp2-table32 ASM. Not production.
...
const previous=require('./pvq_exp2_table32.cjs'),sem=require('./pvq_qn_table_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),{inspect:layout}=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-qn-table-'+t+'-v1';
...
const dependencies=['pvq_qn_table_proof.cjs','pvq_qn_table.s','pvq_exp2_table32.cjs','pvq_exp2_table32_proof.cjs','frozen_reloads.cjs','report_layout.cjs'];
...
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'pvq_qn_table.s'),'utf8').replace(/\r\n/g,'\n');
function prove(functions,actual){for(const n of names.filter(n=>n!=='quant_partition'))assert.deepEqual(functions[n],actual[n]);return sem.prove(functions.quant_partition,actual.quant_partition);}
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_qn_table_variant,'control');assert.equal(b.post_link_pvq_qn_table_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_qn_table_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' exact v+1 qn lookup raw benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_qn_table_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
 ...
 const semantic=prove(info.functions,actual.functions);sem.tableProof(a,false);const table=sem.tableProof(b,true);
 const literal={before:sem.literalProof(a,info.functions.time_service_poll,false),after:sem.literalProof(b,actual.functions.time_service_poll,true)};
 const proof={schema:1,parent,parent_proof_sha256:hash(fs.readFileSync(path.join(dir,'preflight.json'))),recipe_sha256_lf:sourceHash(__filename),dependencies:depHashes(),parent_elf_sha256:hash(a),candidate_elf_sha256:hash(b),patches,semantic,literal,table,static_ram_delta:0,stack_delta:0,app_bytes:app.length,sections:actual.sections,functions:info.functions,actual_functions:actual.functions,imageProof:base.compareApps(images.control,images.candidate,patches),packaging:{esptool_sha256:hash(fs.readFileSync(esptool)),logs}};
 ...
 console.log(JSON.stringify({passed:true,sha256:manifests.candidate.app_sha256,app_bytes:app.length,ram_delta:0,patches:patches.length,cases:semantic.cases}));return proof;
}
function verifyPair(){
 const dir=art('candidate'),p=read(path.join(dir,'preflight.json'));assert.equal(p.recipe_sha256_lf,sourceHash(__filename));assert.deepEqual(p.dependencies,depHashes());
 ...
 assert.deepEqual(prove(p.functions,p.actual_functions),p.semantic);sem.tableProof(a,false);assert.deepEqual(sem.tableProof(b,true),p.table);
 assert.deepEqual({before:sem.literalProof(a,p.functions.time_service_poll,false),after:sem.literalProof(b,p.actual_functions.time_service_poll,true)},p.literal);
...
function cModels(){
 const dir=path.join(root,'.build/opus-bands-pvq-qn-table');fs.mkdirSync(dir,{recursive:true});const {once}=require('./bands.cjs');
 return previous.cModels().map(model=>{let text=fs.readFileSync(model.file,'utf8').replace(/\r\n/g,'\n');
  text=once(text,'static int compute_qn(',`static const opus_int32 y_pvq_qn_table[65] = {${sem.values.join(',')}};\nstatic int compute_qn(`);
  text=once(text,'qn = (stereo ? exp2_table8[qb&0x7] : y_pvq_exp2_table32[qb&0x7])>>(14-(qb>>BITRES));\n      qn = (qn+1)>>1<<1;',
   'if (stereo) { qn = exp2_table8[qb&0x7]>>(14-(qb>>BITRES)); qn = (qn+1)>>1<<1; }\n      else qn = y_pvq_qn_table[qb] & ~1;');
  const file=path.join(dir,'bands.model.c');fs.writeFileSync(file,text);return{source:model.source,file};});
}
module.exports={parent,variant,art,build,names,read,depHashes,sourceFor,prove,manifestPair,generate,verifyPair,cModels};
