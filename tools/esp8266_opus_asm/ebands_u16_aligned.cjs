...
const previous=require('./ebands_final.cjs'),sem=require('./ebands_u16_aligned_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-ebands-u16-aligned-'+t+'-v1';
...
const dependencies=['ebands_u16_proof.cjs','ebands_u16_aligned_proof.cjs','ebands_u16_aligned.s','ebands_final.cjs','ebands_final_proof.cjs','ebands_pair_proof.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'ebands_u16_aligned.s'),'utf8').replace(/\r\n/g,'\n');
function manifestPair(a,b){
 assert.equal(a.post_link_ebands_u16_aligned_variant,'control');assert.equal(b.post_link_ebands_u16_aligned_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_ebands_u16_aligned_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' aligned nonnegative eBands extraction raw benchmark; not production',app_sha256:hash(images[t]),post_link_ebands_u16_aligned_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
 ...
 const helperDisassembly=sem.specs.map((s,i)=>layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'ebands_u16_aligned_'+i,address:s.helper,bytes:s.span}));
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-ebands-u16-aligned');fs.mkdirSync(dir,{recursive:true});
