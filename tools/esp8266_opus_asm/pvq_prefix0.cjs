...
const previous=require('./ebands_final.cjs'),sem=require('./pvq_prefix0_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-prefix0-'+t+'-v1';
...
const dependencies=['pvq_prefix0.s','pvq_prefix0_proof.cjs','pvq_binary_proof.cjs','quant_decode_proof.cjs','audit_quant_decode.cjs','ebands_final.cjs','report_layout.cjs','frozen_reloads.cjs'];
const depHashes=()=>Object.fromEntries(dependencies.map(n=>[n,sourceHash(path.join(__dirname,n))]));
const sourceFor=t=>fs.readFileSync(path.join(__dirname,'pvq_prefix0.s'),'utf8').replace(/\r\n/g,'\n').replace('@TABLE_BYTES@',Array.from({length:11},(_,i)=>'    .byte '+[...sem.prefix(t).subarray(i*32,i*32+32)].join(',')).join('\n'));
...
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_prefix0_variant,'control');assert.equal(b.post_link_pvq_prefix0_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_prefix0_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 save(asm,source);save(script,'pvq_prefix0_fast = 0x4025335d;\npvq_prefix0_flags = 0x40253354;\nSECTIONS {\n'+patches.map((p,i)=>` .text.patch${i} 0x${p.address.toString(16)} : { *(.text.patch${i}) }\n`).join('')+'}\n');
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Fixed-address '+t+' exact PVQ packed exponent-prefix row search raw ASM benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_prefix0_variant:t,
 ...
 const helperDisassembly=layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'pvq_prefix0_leaf',address:sem.helper,bytes:sem.helperBytes});
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-pvq-prefix0');fs.mkdirSync(dir,{recursive:true});
 ...
 text='static const unsigned char y_pvq_prefix0[352] = {'+[...sem.prefix(t)].join(',')+'};\n'+text;
 const anchor='else for(p=row[_k];p>_i;p=row[_k])_k--;';assert.equal(text.split(anchor).length-1,1);
 text=text.replace(anchor,`else {
        if(_k-_n>=8){
          unsigned bound=y_pvq_prefix0[(_n-3)*32+__builtin_clz(_i)];
