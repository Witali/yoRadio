...
const previous=require('./ebands_final.cjs'),sem=require('./pvq_tree8_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-tree8-'+t+'-v1';
const art=t=>path.join(root,'firmware/development',variant(t)),build=t=>path.join(root,'.build',variant(t)),read=previous.read;
const names=[...new Set([...previous.names,'celt_decode_with_ec_dred'])];
...
const dependencies=['pvq_tree8.s','pvq_tree8_proof.cjs','pvq_binary_proof.cjs','quant_decode_proof.cjs','audit_quant_decode.cjs','ebands_final.cjs','report_layout.cjs','frozen_reloads.cjs'];
const depHashes=()=>Object.fromEntries(dependencies.map(n=>[n,sourceHash(path.join(__dirname,n))]));
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'pvq_tree8.s'),'utf8').replace(/\r\n/g,'\n');
...
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_tree8_variant,'control');assert.equal(b.post_link_pvq_tree8_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_tree8_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 const patches=[{address:sem.site,bytes:sem.siteBytes},{address:sem.storageAddress,bytes:sem.storageBytes}],source=sourceFor();
 ...
 save(asm,source);save(script,'pvq_tree8_fast = 0x4025335d;\npvq_tree8_flags = 0x40253354;\nSECTIONS {\n'+patches.map((p,i)=>` .text.patch${i} 0x${p.address.toString(16)} : { *(.text.patch${i}) }\n`).join('')+'}\n');
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Fixed-address '+t+' exact PVQ static tree row search raw ASM benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_tree8_variant:t,
 ...
 const actual=inspect(variant('candidate'),names.filter(n=>n!=='quant_all_bands'));
 assert.deepEqual(actual.sections,info.sections);assert.deepEqual(actual.function_sizes,info.function_sizes);
 for(const n of names.filter(n=>!['quant_all_bands','decode_pulses'].includes(n)))assert.deepEqual(actual.functions[n],info.functions[n]);
 const helperDisassembly=layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'pvq_tree8_leaf',address:sem.helper,bytes:sem.helperBytes});
 const semantic=sem.prove(info.functions.decode_pulses,actual.functions.decode_pulses,helperDisassembly,table);assert.deepEqual(sem.table(b),table);
 ...
 const proof={schema:1,parent,recipe_sha256_lf:sourceHash(__filename),dependencies:depHashes(),parent_elf_sha256:hash(a),candidate_elf_sha256:hash(b),
  patches,storage,semantic,table,helperDisassembly,static_ram_delta:0,stack_delta:0,app_bytes:app.length,sections:actual.sections,functions:info.functions,actual_functions:actual.functions,
...
function verifyPair(){
 ...
 assert.deepEqual(p.patches.map(({address,bytes})=>({address,bytes})),[{address:sem.site,bytes:sem.siteBytes},{address:sem.storageAddress,bytes:sem.storageBytes}]);
 ...
 assert.deepEqual(sem.table(a),p.table);assert.deepEqual(sem.table(b),p.table);
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-pvq-tree8');fs.mkdirSync(dir,{recursive:true});
 ...
 text=text.replace(anchor,`else {
        if(_k-_n<8)for(p=row[_k];p>_i;p=row[_k])_k--;
        else {
          int k=_k;
          p=row[k-3];
          if(_i>=p){
            q=row[k-1];
            if(_i>=q){p=row[k];if(_i<p){p=q;_k=k-1;}}
            else {q=row[k-2];if(_i>=q){p=q;_k=k-2;}else _k=k-3;}
          }else{
            p=row[k-5];
            if(_i>=p){q=row[k-4];if(_i>=q){p=q;_k=k-4;}else _k=k-5;}
            else{
              p=row[k-7];
              if(_i>=p){q=row[k-6];if(_i>=q){p=q;_k=k-6;}else _k=k-7;}
              else{_k=k-8;for(p=row[_k];p>_i;p=row[_k])_k--;}
            }
          }
        }
      }`);
