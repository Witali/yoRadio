// Last PVQ probe shortcut on the accepted baseline; not production.
...
const sem=require('./bits_fifth_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),{inspect}=require('./report_layout.cjs');
const parent=pvq.variant('candidate'),variant=t=>'esp8266-opus-bits-fifth-'+t+'-v1';
...
const dependencies=['bits_fifth_proof.cjs','bits_fifth.s','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs'];
...
function sourceFor(){return fs.readFileSync(path.join(__dirname,'bits_fifth.s'),'utf8').replace(/\r\n/g,'\n');}
function prove(functions,actual,patches){
 const search=sem.prove(functions.quant_partition,actual.quant_partition);
 const contract=require('./partition_frozen_proof.cjs').contract({functions});
 for(const n of names.slice(1))assert.deepEqual(actual[n],functions[n]);
 return{search,contract};
}
function manifestPair(a,b){
 assert.equal(a.post_link_bits_fifth_variant,'control');assert.equal(b.post_link_bits_fifth_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_bits_fifth_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){
...
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' last PVQ probe shortcut ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_bits_fifth_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
...
function cModels(){
 const dir=path.join(root,'.build/opus-bands-bits-fifth');fs.mkdirSync(dir,{recursive:true});
