// Frozen-layout ASM experiment over accepted exp2-table32; not production.
...
const previous=require('./pvq_exp2_table32.cjs'),sem=require('./ebands_pair_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-ebands-pair-'+t+'-v1';
const art=t=>path.join(root,'firmware/development',variant(t)),build=t=>path.join(root,'.build',variant(t)),read=previous.read;
const names=[...previous.names,'clt_compute_allocation'],inspect=v=>layout.inspect(v,names,{internalCalls:['quant_partition']});
...
const dependencies=['ebands_pair_proof.cjs','ebands_pair.s','pvq_exp2_table32.cjs','pvq_logn_word_proof.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
const sourceFor=()=>fs.readFileSync(path.join(__dirname,'ebands_pair.s'),'utf8').replace(/\r\n/g,'\n');
function manifestPair(a,b){
 assert.equal(a.post_link_ebands_pair_variant,'control');assert.equal(b.post_link_ebands_pair_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_ebands_pair_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){assert.equal(m.post_link_pvq_exp2_table32_variant,'candidate');assert.equal(m.opus_backend,'bands-tell-inline-asm');assert.equal(m.cpu_mhz,160);assert.equal(m.flash,'QIO40');assert.equal(m.opus_benchmark,true);assert.equal(m.opus_benchmark_output,false);assert.equal(m.opus_function_profile,false);assert.equal(m.opus_profile_stage,0);}
}
function generate(){
 ...
 const patches=sem.findPatches(info.functions),source=sourceFor();previous.disjoint(patches);
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' eBands pair raw benchmark; not production',app_sha256:hash(images[t]),post_link_ebands_pair_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
 ...
 const helperDisassembly=layout.reachableDisassembly(path.join(build('candidate'),base.elfName),{name:'ebands_pair',address:sem.helper,bytes:sem.helperBytes});
 ...
 const proof={schema:1,parent,recipe_sha256_lf:sourceHash(__filename),dependencies:depHashes(),parent_elf_sha256:hash(a),candidate_elf_sha256:hash(b),original_storage:require('./pvq_logn_word_proof.cjs').storageBytesProof(a),patches,semantic,table,helperDisassembly,static_ram_delta:0,stack_delta:0,app_bytes:app.length,sections:actual.sections,functions:info.functions,actual_functions:actual.functions,imageProof:base.compareApps(images.control,images.candidate,patches),packaging:{esptool_sha256:hash(fs.readFileSync(esptool)),logs}};
...
function verifyPair(){
 ...
 assert.deepEqual(require('./pvq_logn_word_proof.cjs').storageBytesProof(a),p.original_storage);
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-ebands-pair');fs.mkdirSync(dir,{recursive:true});
 const input=fs.readFileSync(path.join(component,'upstream/celt/rate.c'),'utf8').replace(/\r\n/g,'\n');
 const anchor='         int N = m->eBands[j+1]-m->eBands[j];';assert.equal(input.split(anchor).length,2);
 const helper=`#include <string.h>\nstatic int y_ebands_width(const CELTMode *m,int j) {\n  opus_uint32 a,b;\n  celt_assert(m->nbEBands==21 && j>=0 && j<21);\n  const unsigned off=(unsigned)j*2,word=off&~3u;\n  memcpy(&a,(const unsigned char*)m->eBands+word,4);\n  if(off&2) { memcpy(&b,(const unsigned char*)m->eBands+word+4,4); a>>=16; }\n  else { b=a>>16; a&=65535; }\n  b&=65535;\n  return (b<32768?(int)b:(int)b-65536)-(a<32768?(int)a:(int)a-65536);\n}\n`;
 const {once}=require('./bands.cjs');let text=once(input,'int clt_compute_allocation(',helper+'int clt_compute_allocation(');text=once(text,anchor,'         int N = y_ebands_width(m,j);');
 const file=path.join(dir,'rate.model.c');fs.writeFileSync(file,text);return[...models,{source:'upstream/celt/rate.c',file}];
}
