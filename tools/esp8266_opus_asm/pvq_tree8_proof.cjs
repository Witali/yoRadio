// Frozen-layout PVQ static tree search: real linked instructions and real U table.
...
const site=0x40253329,siteBytes=43,done=0x40253361,helper=0x402521cc,helperBytes=122,storageAddress=0x402521cb,storageBytes=123;
...
function auditSite(fn,candidate=false){
 ...
 assert.deepEqual(inside(fn).map(clean),candidate?['addi a15, a15, -4','sub a8, a12, a13','bgei a8, 8, 40253351','l32i a8, a15, 0','bgeu a2, a8, 4025335d','addi a6, a15, -4','l32i a8, a6, 0','addi a6, a6, -4','addi a12, a12, -1','bltu a2, a8, 4025333a','addi a6, a13, -1','j 40253354','call0 402521cc']:oldOps);
 ...
 assert.deepEqual(rows.filter(r=>/^call/.test(r.op)).map(r=>[r.address,clean(r)]),candidate?
  [[0x40253213,'call0 40246c20'],[site+40,'call0 402521cc']]:[[0x40253213,'call0 40246c20']]);
...
function prove(before,after,helperText,t){
 ...
 // Preserve every original instruction outside the site, including fast exits.
 ...
 const visited=new Set();let cases=0,linearReads=0,treeReads=0,maxSteps=0;
  ...
  for(let K=n;K<n+values.length-1;K++){
   ...
   for(let tail=n;tail<=K;tail++){
    ...
    for(const index of new Set([low,high,low+Math.floor((high-low)/2)]))for(const seed of[0,0xffffffff]){
     ...
     linearReads+=x.reads.length;treeReads+=y.reads.length;maxSteps=Math.max(maxSteps,y.steps);cases++;
    }
   }
  }
 }
 for(const row of h)assert.ok(visited.has(row.address),'Uncovered helper instruction');
 return{cases,helper_instructions_covered:h.length,linear_reads:linearReads,tree_reads:treeReads,max_candidate_instructions:maxSteps,
  stack_bytes:48,stack_delta:0,static_ram_delta:0,stores:0,sar_writes:0,
  invariant:'If K-N<8, keep the exact descending linear search. Otherwise ordered comparisons at K-3, K-1/K-5, then neighboring thresholds select d=0..7 and cached p=row[K-d], or prove d>=8 and continue linearly at K-8. The K-N guard makes all probes valid. The unchanged index>=row[N] guard terminates the tail. No division, midpoint, RAM table, stores or SAR changes.',
...
module.exports={site,siteBytes,done,helper,helperBytes,storageAddress,storageBytes,inside,auditSite,storage,table,emulate,checkHelper,prove};
