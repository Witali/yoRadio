...
const previous=require('./ebands_final.cjs'),sem=require('./quant_flags_proof.cjs');
const {sections,inspectImage}=require('./frozen_div.cjs'),layout=require('./report_layout.cjs');
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-quant-flags-'+t+'-v1';
...
const dependencies=['quant_flags_proof.cjs','quant_decode_proof.cjs','audit_quant_decode.cjs','ebands_final.cjs','report_layout.cjs','frozen_reloads.cjs'];
...
function manifestPair(a,b){
 assert.equal(a.post_link_quant_flags_variant,'control');assert.equal(b.post_link_quant_flags_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_quant_flags_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
...
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Same-address '+t+' private flag load quant_all_bands raw ASM benchmark; not production',app_sha256:hash(images[t]),post_link_quant_flags_variant:t,
...
function cModels(){
 const models=previous.cModels(),dir=path.join(root,'.build/opus-bands-quant-flags');fs.mkdirSync(dir,{recursive:true});
