// Exact row-length word helper on accepted a4-word ASM; not production.
...
const base=require('./frozen_reloads.cjs'),previous=require('./pvq_a4_word.cjs');
const sem=require('./pvq_row_word_proof.cjs');
...
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-row-word-'+t+'-v1';
...
const dependencies=['pvq_row_word_proof.cjs','pvq_row_word.s','pvq_a4_word_proof.cjs','pvq_byte_word_proof.cjs','bits_fifth_proof.cjs','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs','report_layout.cjs'];
...
function sourceFor(){return fs.readFileSync(path.join(__dirname,'pvq_row_word.s'),'utf8').replace(/\r\n/g,'\n');}
...
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_row_word_variant,'control');assert.equal(b.post_link_pvq_row_word_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_row_word_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){
  assert.equal(m.post_link_pvq_a4_word_variant,'candidate');assert.equal(m.post_link_pvq_byte_word_variant,'candidate');assert.equal(m.post_link_endpoint_cost_variant,'candidate');assert.equal(m.post_link_bits_fourth_variant,'candidate');assert.equal(m.post_link_bits_fifth_variant,'candidate');assert.equal(m.post_link_pvq_row_loop_variant,'candidate');assert.equal(m.post_link_post_pair_variant,'candidate');
...
function tableProof(elf) {
 ...
 return {address:sem.tableBase,bytes:sem.tableBytes,sha256:hash(bytes),mode_address:modeAddress,mode_bits_offset:92,index_address:indexAddress,index_bytes:indexBytes.length,index_sha256:hash(indexBytes),all_aligned_words_within_table:true};
}
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' word-load PVQ byte helper ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_row_word_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
...
function cModels(){
 const dir=path.join(root,'.build/opus-bands-pvq-row-word');fs.mkdirSync(dir,{recursive:true});
 const {once}=require('./bands.cjs');
 return previous.cModels().map(model=>{
  let text=fs.readFileSync(model.file,'utf8').replace(/\r\n/g,'\n');
  text=once(text,'int LM,int bits,int *cost) {','int LM,int bits,int row_length,int *cost) {');
  text=once(text,'int lo=0,hi=cache[0];bits--;','int lo=0,hi=row_length;bits--;');
  const anchor='   cache = m->cache.bits + m->cache.index[(LM+1)*m->nbEBands+i];\n';
  text=once(text,anchor,anchor+'   const int y_row_length=y_pvq_word8(m,cache);\n');
  text=once(text,'cache+cache[0]','cache+y_row_length');
  text=once(text,'y_endpoint_cost(m, i, LM, b, &curr_bits)','y_endpoint_cost(m, i, LM, b, y_row_length, &curr_bits)');
