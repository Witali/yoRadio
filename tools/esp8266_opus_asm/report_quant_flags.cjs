// Fresh raw A/B/A for the fixed-address flag experiment; retain every attempt.
const fs=require('node:fs'),path=require('node:path'),assert=require('node:assert/strict');
const {root}=require('./export.cjs'),f=require('./quant_flags.cjs');
...
const dest=f.art('candidate'),experiment=path.join(root,'.build/opus-quant-flags-board-20260918');
function report(){
