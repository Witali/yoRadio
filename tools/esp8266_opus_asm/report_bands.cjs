...
function report(kind){
 assert.ok(['intensity','blocks','combined','pulse-lookup','tell-inline','fused','tell-intensity','update-fast'].includes(kind));
 const fresh=['pulse-lookup','tell-inline','fused','tell-intensity','update-fast'].includes(kind);
 ...
 const identity=['opus_backend','opus_asm_optimization_sha256','opus_bands_blocks_manifest_sha256','opus_bands_combined_manifest_sha256','opus_bands_pulse_lookup_manifest_sha256','opus_bands_tell_inline_manifest_sha256','opus_bands_fused_manifest_sha256','opus_bands_tell_intensity_manifest_sha256','opus_bands_update_fast_manifest_sha256','built_utc','source_revision','app_sha256','bytes'];
 ...
 const repeated=path.join(root,['tell-inline','fused','update-fast'].includes(seriesKind)?'.build/opus-'+seriesKind+'-control-repeat-20260913':'.build/opus-bands-control-repeat-20260913');
 if((!fresh||['tell-inline','fused','update-fast'].includes(seriesKind))&&fs.existsSync(path.join(repeated,'run10.json'))&&read(path.join(repeated,'run10.json')).final?.state===3){
  const a2=archive(repeated,path.join(control,['tell-inline','fused','update-fast'].includes(seriesKind)?seriesKind+'-repeat-20260913':'repeated'));result.repeated_control=compare(a2.map(v=>v.report),b.map(v=>v.report));
 ...
 if(['tell-inline','tell-intensity'].includes(kind)){
  ...
  result.build.symbols={reference:symbols(av),candidate:symbols(bv)};
  assert.deepEqual(result.build.symbols.candidate.y_opus_tell_correction,result.build.symbols.candidate['correction$2307']);
  assert.equal(result.build.symbols.candidate.y_opus_tell_correction.bytes,32);
 }
