// Phase-specialized signed logN access on accepted index-half ASM; not production.
...
const sem=require('./pvq_logn_word_proof.cjs');
...
const parent=previous.variant('candidate'),variant=t=>'esp8266-opus-pvq-logn-word-'+t+'-v1';
...
const dependencies=['pvq_index_word_proof.cjs','pvq_index_half_proof.cjs','pvq_logn_word_proof.cjs','pvq_logn_word.s','pvq_endpoint_word_proof.cjs','frozen_reloads.cjs','pvq_row_word_proof.cjs','pvq_a4_word_proof.cjs','pvq_byte_word_proof.cjs','bits_fifth_proof.cjs','partition_frozen_proof.cjs','partition_decode.cjs','pulse_lookup.cjs','pulse_inverse.cjs','report_layout.cjs'];
...
function sourceFor(){return fs.readFileSync(path.join(__dirname,'pvq_logn_word.s'),'utf8').replace(/\r\n/g,'\n');}
function prove(functions,actual,patches){
 const search=sem.prove(functions.quant_partition,actual.quant_partition);
 const contract={scope:'Unchanged caller chain and frame. Phase-specific logN helper preserves every register except original signed a4 result, including a0/a10/SAR. Parent private decoder contract retained.'};
 for(const n of names.slice(1))assert.deepEqual(actual[n],functions[n]);
 return{search,contract};
}
function manifestPair(a,b){
 assert.equal(a.post_link_pvq_logn_word_variant,'control');assert.equal(b.post_link_pvq_logn_word_variant,'candidate');
 for(const k of new Set([...Object.keys(a),...Object.keys(b)]))if(!['purpose','app_sha256','post_link_pvq_logn_word_variant'].includes(k))assert.deepEqual(a[k],b[k],k);
 for(const m of[a,b]){
...
function tableProof(elf) {
 const inherited=previous.tableProof(elf);
 ...
 assert.equal(readAt(0x402d3968+48,4).readUInt32LE(),sem.logBase,'Static mode logN pointer');
 const raw=readAt(sem.logBase,sem.logBytes),values=sem.logValues.map((_,i)=>raw.readInt16LE(i*2));assert.deepEqual(values,sem.logValues);
 assert.equal(sem.logBase%4,0);const padding=readAt(sem.logBase+sem.logBytes,2);assert.deepEqual(padding,Buffer.alloc(2),'Final logN word requires authenticated padding');
 const words=readAt(sem.logBase,sem.logBytes+2);
 return {...inherited,logn_address:sem.logBase,logn_bytes:sem.logBytes,logn_sha256:hash(raw),logn_word_bytes:words.length,logn_word_sha256:hash(words),logn_padding_hex:padding.toString('hex')};
}
function generate(){
 ...
 for(const[t,elf]of[['control',a],['candidate',b]]){
  ...
  manifests[t]={...m,purpose:'Frozen-layout '+t+' signed logN word helper ASM raw benchmark; not production',app_sha256:hash(images[t]),post_link_pvq_logn_word_variant:t,post_link_recipe_sha256_lf:sourceHash(__filename),post_link_bundle_dependencies:depHashes(),post_link_parent_app_sha256:hash(app),post_link_parent_elf_sha256:hash(a)};
 }
...
// Match only the non-stereo theta clone in quant_partition; other clones unchanged.
// Do not read beyond the C array at its last halfword; target padding is ELF-proven.
function cModels(){
 const dir=path.join(root,'.build/opus-bands-pvq-logn-word');fs.mkdirSync(dir,{recursive:true});
...
static int y_pvq_logn_word(const CELTMode *m,int i) {
  opus_uint32 word=0;
  celt_assert(m->nbEBands==21 && i>=0 && i<21);
  const unsigned offset=(unsigned)i*2,aligned=offset&~3u;
  memcpy(&word,(const unsigned char*)m->logN+aligned,aligned+4<=42?4:2);
...
`;
  ...
  text=once(text,'   pulse_cap = m->logN[i]+LM*(1<<BITRES);',
   '   pulse_cap = (stereo ? m->logN[i] : y_pvq_logn_word(m,i))+LM*(1<<BITRES);');
