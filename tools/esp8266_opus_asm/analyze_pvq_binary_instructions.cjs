...
const {root,hash,sourceHash}=require('./export.cjs'),f=require('./pvq_binary.cjs'),sem=require('./pvq_binary_proof.cjs'),base=require('./frozen_reloads.cjs');
function analyze(){
 ...
 for(const c of census.cases){
  ...
  assert.equal(rows.length,c.counts.rows);assert.equal(oldReads,c.counts.table_probes.linear);assert.equal(newReads,c.counts.table_probes.binary);
