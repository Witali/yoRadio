// Fresh raw A/B/A for the fixed-address PVQ static tree search experiment; retain every attempt.
const fs=require('node:fs'),path=require('node:path'),assert=require('node:assert/strict');
const {root}=require('./export.cjs'),f=require('./pvq_tree8.cjs');
...
const dest=f.art('candidate'),experiment=path.join(root,'.build/opus-pvq-tree8-board-20260919');
