// Exact signed logN extraction, fixed continuation, no live-register clobber.
...
const helper=0x4024dcc4,helperBytes=42,liveBytes=25,site=0x4024db91,continuation=site+3;
const logBase=0x402d5a58,logBytes=42,logValues=[0,0,0,0,0,0,0,0,8,8,8,8,16,16,16,21,21,24,29,34,36];
...
const definitions=()=>`pvq_logn_word = 0x${helper.toString(16)};\npvq_logn_continue = 0x${continuation.toString(16)};`;
...
function storageProof(fn){
 ...
 assert.equal(predecessor.address+predecessor.bytes,helper);assert.equal(predecessor.text,'sub a2, a2, a3');assert.ok(dead.has(predecessor.address),'Live fallthrough into storage');
...
function storageBytesProof(elf){
 ...
 assert.deepEqual(elf.subarray(b,b+helperBytes),bytes,'Original logN helper storage changed');
...
function findPatches(fn){
 ...
 const rows=base.parsed(fn),load=rows.find(r=>r.address===site);assert.equal(load?.bytes,3);assert.equal(load.text,'l16si a4, a10, 0');
...
function helperSymbolic(fn){
 ...
 assert.equal(rows[0].op,'bbci');assert.deepEqual(rows[0].operands,['a10','1',(helper+14).toString(16)]);
 assert.deepEqual(rows.slice(1,4).map(r=>r.text),['addi a4, a10, -2','l32i a4, a4, 0','srai a4, a4, 16']);
 assert.deepEqual(rows.slice(5,8).map(r=>r.text),['l32i a4, a10, 0','slli a4, a4, 16','srai a4, a4, 16']);
 ...
 for(const phase of[0,2]){
  const word=Array.from({length:32},(_,i)=>'W'+i),r=Array.from({length:16},(_,i)=>'R'+i);r[10]=logBase+phase;const initial=r.slice();let loads=0;
  ...
  for(const row of trace){const op=row.op,a=row.operands;if(op==='bbci'||op==='j')continue;const d=reg(a[0]),s=reg(a[1]);
   ...
   else if(op==='l32i'){assert.equal(r[s]+Number(a[2]),logBase);r[d]=word;loads++;}
  ...
  assert.deepEqual(r[4],word.slice(phase*8,phase*8+16).concat(Array(16).fill(word[phase*8+15])));
  for(let i=0;i<16;i++)if(i!==4)assert.equal(r[i],initial[i]);assert.equal(loads,1);
  records.push({phase,result:r[4],loads,executed:trace.length});
 }
 return{records,instructions:9,executed_per_phase:5,live_bytes:liveBytes,sar:'untouched',scope:'Every word bit, both int16 phases, all GPRs except signed result a4 unchanged, including a0/a10; no SAR or stack instruction.'};
}
function execute(fn,initial,data,sar){
 ...
 let pc=site,steps=0,words=0,shorts=0;assert.equal(r[10]%2,0,'Unaligned source');assert.ok(r[10]>=logBase&&r[10]+2<=logBase+logBytes,'Source outside logN');
 while(pc!==continuation){assert.ok(++steps<10);const row=map.get(pc);assert.ok(row,'Unknown PC');visited.push(pc);let next=pc+row.bytes;const op=row.op,a=row.operands;
  if(op==='j'){assert.ok(pc===site||helperInside(pc));next=prior.target(row);}
   ...
   else if(op==='l16si'||op==='l32i'){const address=r[s]+Number(a[2]),off=address-logBase,bytes=op==='l16si'?2:4;assert.ok(off>=0&&off+bytes<=data.length,'Word outside storage');
   ...
   else assert.fail('Unsupported logN instruction '+op);
  }pc=next;
 }
 return{registers:r,visited,steps,words,shorts,sar,pc};
}
function numeric(oldFn,newFn){
 const a=new Map(base.parsed(oldFn).map(r=>[r.address,r])),b=new Map(base.parsed(newFn).map(r=>[r.address,r])),data=Buffer.alloc(44);
 const result={signed_cases:0,table_cases:0,word_loads:0,old_steps:0,new_steps:0};
 const check=(off,sar,kind,seed)=>{
  const r=Array.from({length:16},(_,i)=>Math.imul(seed+i+1,0x45d9f3b)>>>0);r[1]=0x3fffef00;r[10]=logBase+off;
 ...
 logValues.forEach((v,i)=>data.writeInt16LE(v,i*2));data.writeUInt16LE(0,42);
 for(let i=0;i<21;i++)for(let sar=0;sar<64;sar++)check(i*2,sar,'table',i*64+sar);
 return{...result,scope:'All65536 signed values/two phases,21 actual logN values/all64 SAR states. Five extra visible instructions replace the emulated narrow read; no cycle estimate.'};
}
function prove(oldFn,newFn){
 const patches=findPatches(oldFn),a=base.parsed(oldFn),b=base.parsed(newFn);
 assert.equal(oldFn.address,newFn.address);assert.equal(oldFn.bytes,newFn.bytes);assert.deepEqual(a.filter(r=>!inside(r.address)),b.filter(r=>!inside(r.address)),'Outside logN patches changed');
 ...
 assert.equal(b.find(r=>r.address===continuation)?.text,'slli a2, a5, 3');assert.equal(b.find(r=>r.address===0x4024db36)?.text,'s32i a0, a1, 108');
...
module.exports={helper,helperBytes,liveBytes,site,continuation,logBase,logBytes,logValues,inside,helperInside,definitions,storageProof,storageBytesProof,findPatches,helperSymbolic,execute,numeric,prove};
