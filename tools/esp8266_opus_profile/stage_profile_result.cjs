...
function analyzeStage(status,item) {
  ...
  for(const name of ['stage_cycles','stage_calls','stage_max_cycles'])
    if(!Number.isSafeInteger(item[name])||item[name]<0)throw Error('Invalid '+name);
  if(item.stage_max_cycles>item.stage_cycles||(!item.stage_calls&&item.stage_cycles))throw Error('Inconsistent stage counters');
  const us=item.stage_cycles/(status.stage_clock_hz/1000000);
