...
Во всех файлах станции сортируются сначала по короткому имени источника (`101.ru`, `Caprice`, `Record` и так далее), затем по имени станции. В `playlist.m3u` это же имя источника записывается в `group-title`.
