...
if (-not $BdfPath) {
    $BdfPath = Join-Path $PSScriptRoot "fonts\spleen-8x16.bdf"
}
if (-not $OutputPath) {
    $OutputPath = Join-Path $repository `
        "yoRadio\src\displays\fonts\C3Terminal15.h"
}
if (-not $NativeOutputPath) {
    $NativeOutputPath = Join-Path $repository `
        "idf\esp32c3-oled-native\main\font8x15.h"
}
if (-not (Test-Path -LiteralPath $BdfPath -PathType Leaf)) {
    throw "Spleen 8x16 BDF was not found at $BdfPath"
}
...
function Set-ReplacementGlyph {
    ...
    for ($y = 0; $y -lt $cellHeight; $y++) {
        for ($x = 0; $x -lt $cellWidth; $x++) {
            $border = ($y -le 14) -and `
                ($x -eq 0 -or $x -eq 7 -or $y -eq 0 -or $y -eq 14)
            $question = ($y -eq 3 -and $x -ge 2 -and $x -le 5) -or `
                ($y -eq 4 -and $x -eq 6) -or `
                (($y -eq 5 -or $y -eq 6) -and $x -eq 5) -or `
                (($y -eq 7 -or $y -eq 10) -and $x -eq 4)
...
for ($code = 0; $code -le 255; $code++) {
    ...
    $glyphLines.Add((
        "  {{ {0,5}, 8, 15, 8, 0, -12 }}, // 0x{1:X2}" -f $offset, $code))
}
...
$lines.Add("#ifndef C3_TERMINAL_15_H")
$lines.Add("#define C3_TERMINAL_15_H")
$lines.Add("")
foreach ($comment in $licenseComment) { $lines.Add($comment) }
$lines.Add("const uint8_t C3Terminal15Bitmaps[] PROGMEM = {")
...
$lines.Add("};")
$lines.Add("")
$lines.Add("const GFXglyph C3Terminal15Glyphs[] PROGMEM = {")
foreach ($glyph in $glyphLines) { $lines.Add($glyph) }
$lines.Add("};")
$lines.Add("")
$lines.Add("const GFXfont C3Terminal15 PROGMEM = {")
$lines.Add("  (uint8_t *)C3Terminal15Bitmaps,")
$lines.Add("  (GFXglyph *)C3Terminal15Glyphs,")
$lines.Add("  0x00, 0xFF, 16")
$lines.Add("};")
$lines.Add("")
$lines.Add("#endif")
...
$nativeLines.Add("static const uint8_t font8x15[$($bitmapBytes.Count)] = {")
for ($index = 0; $index -lt $bitmapBytes.Count; $index += 12) {
    $last = [Math]::Min($index + 11, $bitmapBytes.Count - 1)
    $chunk = for ($item = $index; $item -le $last; $item++) {
        '0x{0:X2}' -f $bitmapBytes[$item]
...
$nativeLines.Add("};")
$nativeLines.Add("")
$nativeLines.Add("static const uint32_t font8x15_unicode_80_bf[64] = {")
