...
function build(directory) {
  ...
  const exe=path.join(directory,process.platform==='win32'?'rcpdm8-quality.exe':'rcpdm8-quality');
  const includes=[directory,path.join(root,'tests/native'),path.join(root,'esp8266/rtos-sdk-native/main')];
  const file=path.join(__dirname,'rcpdm8_quality.cpp');
  let result;
...
if(require.main===module) console.log(build(path.resolve(process.argv[2]||'radio_output/rcpdm8-quality/build')));
