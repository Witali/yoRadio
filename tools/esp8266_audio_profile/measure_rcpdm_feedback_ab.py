...
def generate(exe, pcm, prefix, seed):
    proof = json.loads(subprocess.check_output([str(exe), str(pcm), str(prefix), str(seed)], text=True))
...
def lsb_analysis(exe, output):
    ...
    for seed in (1, 8266, SEED):
        ...
        for name, file in files.items():
            paths, proof = generate(exe, file, output / f"{name}-{seed}", seed)
            if model.original.sha(paths["on"]) != old["measurements"][name]["bitstream_sha256"]:
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "radio_output/rcpdm-feedback-ab-20260906")
    ...
    configs = {mode: dict(bits=32, bit_rate_hz=CARRIER, shift=4, alpha=1/16,
        dither=2, interpolate=True, feedback_enabled=mode == "on") for mode in MODES}
    ...
    for number, old in enumerate(cases, 1):
        ...
        if "bitstream_sha256" in old and hashes["on"] != old["bitstream_sha256"]["feedback32"]:
    ...
    result["lsb"] = lsb_analysis(exe, args.output / "lsb")
