...
function evaluate(r) {
  ...
  let quiet;
  if(opus && r.require_autonomous_opus !== false) {
  ...
  return {pass:!errors.length,errors,continuity,quiet,tx_driver_fail_delta:driverDelta,
    ...
    limitation:'No acoustic capture. Boundary requests may perturb output; boot minimum is not a per-station minimum.' +
      (r.require_autonomous_opus === false ? ' Ordinary production: endpoint counters only, no autonomous diagnostic window.' : '')};
}
async function main() {
  ...
  const ordinary=args.includes('--ordinary');
  ...
  const report={date:new Date().toISOString(),base,playlist_sha256:initial.playlist.wire_sha256,
    seconds_without_runner_requests:65,warmup_seconds:12,ordinary,attempts:[]};
  const save=()=>fs.writeFileSync(path.join(dir,'summary.json'),JSON.stringify(report,null,2)+'\n');
  for(let n=0;n<ids.length;n++) {
    const id=ids[n],r={station:id,name:rows[id-1][0],url:rows[id-1][1],date:new Date().toISOString(),require_autonomous_opus:!ordinary};
    report.attempts.push(r);save();
    console.log('START '+(n+1)+'/'+ids.length+' '+id+' '+r.name);
    r.command=await command(base,'play='+id);save();
    await sleep(12000);
    r.before={status:await read(base,'/api/native/status')};
    if(!ordinary)r.before.memory=await read(base,'/api/native/opus-stream');
    ...
    if(!ordinary)r.quiet=await read(base,'/api/native/audio?quiet=1');
    r.after.status=await read(base,'/api/native/status');
    if(!ordinary)r.after.memory=await read(base,'/api/native/opus-stream');
