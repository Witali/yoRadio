...
bool CommandHandler::exec(const char *command, const char *value, uint8_t cid) {
  ...
  if (strEquals(command, "stationuppercase")) { config.saveValue(&config.store.stationUppercase, static_cast<bool>(atoi(value))); display.putRequest(NEWSTATION); netserver.requestOnChange(STATIONNAME, 0); return true; }
  ...
  if (strEquals(command, "mp3decoder")){
    const uint8_t decoder = atoi(value) == 1 ? 1 : 0;
    if(decoder != config.store.mp3Decoder){
      config.saveValue(&config.store.mp3Decoder, decoder);
