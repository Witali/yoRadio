...
ESP32-C3 QEMU machine. It presents the native 72x40 framebuffer through QEMU's
virtual RGB display, sends a deterministic stereo test tone to its audio
...
an SDL display backend can show the scaled OLED framebuffer in a window; the
...
The virtual OLED and PCM devices validate rendered display content and decoded
audio flow, but do not model SSD1306 I2C timing or the physical stereo PDM
