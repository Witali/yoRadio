...
При `-OpusStreamTest` health `/api/native/audio` также содержит
`transport_phase`: 0 idle, 1 DNS, 2 TCP connect, 3 HTTP send, 4 headers,
5 refill, 6 decode, 7 close. `transport_result` — последняя терминальная
причина refill (3 EOF, 4 timeout, -1 ошибка), либо INT32_MIN до первой;
`transport_errno` и `input_bytes` сняты при той же причине перед drain.
Причина сохраняется через reconnect и сбрасывается при новой команде.
Это 16 дополнительных байт только в диагностическом профиле, не счётчик CPU.
Для проверки влияния самого опроса тест непрерывности поддерживает
`--interval-ms 27000`: два снимка всё равно проверяют все накопленные
underruns и длительность PCM, но не показывают подробную фазу внутри окна.
