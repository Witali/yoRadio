...
Статус: план реализации Automatic Light-sleep. Отдельный режим часов в
Deep-sleep уже реализован под явным ключом сборки `-DeepSleepClock`;
см. [описание режима](ESP32C3_DEEP_SLEEP_CLOCK.md). Без ключа он выключен.
...
Режим часов с RTC-пробуждением каждые 500 мс доступен как отдельная опция
сборки `-DeepSleepClock`. Он отключает Wi-Fi/WebUI и проверяет BOOT/GPIO9
при каждом тике. Подробности, сборка и проверки описаны в
[ESP32C3_DEEP_SLEEP_CLOCK.md](ESP32C3_DEEP_SLEEP_CLOCK.md).

Этот режим не реализует требование доступного WebUI из плана Automatic
Light-sleep выше. Без ключа сборки поведение по этому плану не изменяется.
