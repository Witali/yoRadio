...
Новый полный [аудит алгоритма и план ускорения](ESP8266_OPUS_DECODER_SPEED_OPTIMIZATION_PLAN.md)
разделяет применённые изменения, exact-кандидаты и рискованные упрощения.
[Аппаратное сравнение flash → PCM → PDM/DMA](ESP8266_OPUS_FLASH_OUTPUT_BENCHMARK.md)
показывает, что задержки возможны даже без скачивания аудиопотока.
