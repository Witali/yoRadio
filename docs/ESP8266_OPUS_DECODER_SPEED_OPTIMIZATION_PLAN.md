...
### Уточнённая цель пользователя, 2026-09-13

- Opus 192 кбит/с: не более **70% CPU декодера при 160 МГц**.
  Для кадров20мс это средний бюджет14мс вычислений на кадр. Битрейт не
  ограничивать в плеере искусственно;192 — контрольный режим тестирования.
- Снижать расход RAM; не достигать ускорения уменьшением защитного резерва,
  потерей PCM, снижением частоты/точности или исключением плохих измерений.
- Принимать изменение по минимум10 сопоставимых прогонам каждой версии,
  сохраняя median/p95/max, точные PCM-хеши, scratch, DRAM/IRAM и стек.
  Минимальный запас свободной DRAM при воспроизведении —4096Б, стеки не
  уменьшать ниже5120Б. Размер исполняемого кода в IRAM тоже расход RAM.
- Оставшиеся30% — бюджет **всех остальных работ**, не гарантированный idle:
  сеть, контейнер, нормализация, PDM/DMA и WebUI измеряются отдельно.
- После raw-теста обязательно непрерывное физическое воспроизведение
  не менее20с без новых DMA underrun; флаг Playing этого не подтверждает.

Текущая цель **не достигнута**. Последние сопоставимые ASM-прогоны:
93.449% CPU на192; перенос SAR из циклов дал94.833%, поэтому default не
изменён. Новый stage8-профиль относит61.884% decode wall к bands/PVQ и
его вызовам. Подробности и ограничения измерений:
[физический ABBA и профиль192](ESP8266_OPUS_ASM_BOARD_ABBA.md).

### Исторический вывод аудита
...
- [ ] Реальный HTTP Opus ≥20 с без пропусков, затем длительный прогон с
  ...
  TCP timeout, затем stage8: отказ выделения scratch6144B при freeDRAM10220B.
  Исправление интерпретации: это НЕ reserve-guard (он stage10). Размер
  максимального непрерывного блока не измерен: фрагментация/временные сетевые
  аллокации возможны, но не доказаны. После cleanup26992B. Проверить порядок
  повторной инициализации, largest block и CAP8 вместо combined heap в
  reserve-guard. Не уменьшать резерв и не объявлять это доказанной утечкой.
  Пропуски DMA сохраняются отдельно.
...
Уточнение пользователя2026-09-10: **новые физические тесты до192кбит/с**,
а не программный запрет высоких битрейтов. Никакого runtime bitrate limit
не добавлять. Отдельный [корпус12/24/64/128/192](../tests/fixtures/opus_native/through192/README.md)
сохраняет исходные фикстуры и отчёты510. Для нового A/B обе прошивки должны
использовать одинаковый новый корпус; старые результаты не подмешивать.

## Статический аудит воспроизводим

Проверка памяти2026-09-18:
[общая арена ASM и reconnect](ESP8266_OPUS_MEMORY_RECONNECT_2026-09-18.md).

- [x] Устранить лишнее освобождение Opus при первой немеморийной ошибке
  открытия потока; ENOMEM/ENOBUFS/низкая DRAM и окончательная ошибка очищают ресурсы.
- [x] Проверять резерв4096B по CAP8 DRAM, не по combined heap с IRAM.
- [x] Проверить cleanup/reconnect sanitizer-тестами и десятью сетевыми
  отказами на полной ASM-прошивке: decoder-init ошибок в этом прогоне нет.
- [ ] Выяснить остаточную разницу RAM после Stop (~4КБ) целевой трассировкой;
  стабильный остаток за60с ещё не доказывает отсутствие утечек SDK.
- [ ] Устранить оставшиеся CONNECTION ERROR, задержки WebUI и DMA underrun;
  непрерывность звука после исправлений памяти всё ещё не подтверждена.
