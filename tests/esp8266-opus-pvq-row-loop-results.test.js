...
const {root,hash}=require('../tools/esp8266_opus_asm/export.cjs'),f=require('../tools/esp8266_opus_asm/pvq_row_loop.cjs');
...
test('PVQ physical report retains all30 attempts with exact PCM and independently checked CPU medians',()=>{
 const r=read(path.join(dir,'comparison.json')),pair=require('../tools/esp8266_opus_asm/report_pvq_row_loop.cjs').verifyPair();assert.deepEqual(r.pair,pair);
 ...
 for(const k of['initial','repeated']){assert.deepEqual(selectHighBitrate(r[k].cases),r.selection[k]);assert.equal(r.selection[k].accepted_for_experimental_asm,true);assert.equal(r.selection[k].target_192_cpu_at_most_70,false);}
 ...
 assert.deepEqual(r.repeated.reference.observation_errors,[{run:4,error:'request timeout'}]);assert.deepEqual(r.initial.reference.observation_errors,[]);assert.deepEqual(r.initial.candidate.observation_errors,[]);
 assert.deepEqual(r.initial.candidate.timing_window_excesses.map(x=>[x.run,x.id,x.excess_us]),[[2,0,2782],[5,0,1424]]);assert.deepEqual(r.initial.reference.timing_window_excesses,[]);assert.deepEqual(r.repeated.reference.timing_window_excesses,[]);
 assert.equal(Math.min(...groups.candidate[6].final.results.map(v=>v.min_dram)),3816);assert.equal(Math.min(...groups.after[3].final.results.map(v=>v.min_dram)),1400);assert.ok(groups.after[3].final.results[4].task_us*4.8/groups.after[3].final.results[4].samples>100);
 const h=read(path.join(dir,'host-parent.json'));assert.equal(h.passed,true);assert.equal(h.cases.length,11);assert.ok(h.cases.some(c=>c.name==='stereo-510'));assert.ok(h.cases.every(c=>c.pcm.exact&&c.pcm.max_absolute_error===0));
});
test('PVQ experiment restores ordinary radio OTA with matching station, playlist, HTTP and WebSocket',()=>{
