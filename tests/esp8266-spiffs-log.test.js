...
test('SPIFFS logging defaults off and never allocates/writes inside its hook',()=>{
  ...
  assert.doesNotMatch(web,/spiffs_log|api\/native\/log/); // Local/USB only, no HTTP export.
...
test('real SPIFFS logger handles bounded writes, rotation and all injected I/O failures',t=>{
  const dir=fs.mkdtempSync(path.join(os.tmpdir(),'yoradio-spiffs-log-'));
  t.after(()=>fs.rmSync(dir,{recursive:true,force:true}));
  const fixture=fs.readFileSync(path.join(__dirname,'native/esp8266_spiffs_log_test.c'),'utf8');
  const code=fixture.replace('/* LOGGER_IMPLEMENTATION */',source.slice(source.indexOf('#define LOG_FLUSH_MS')));
  const file=path.join(dir,'test.c'),exe=path.join(dir,process.platform==='win32'?'test.exe':'test');
  fs.writeFileSync(file,code);let build;
  ...
  assert.match(run.stdout,/SPIFFS log PASS/);t.diagnostic(run.stdout.trim());
});
