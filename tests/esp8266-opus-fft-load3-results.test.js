...
const {root,hash}=require('../tools/esp8266_opus_asm/export.cjs'),f=require('../tools/esp8266_opus_asm/fft_load3.cjs');
...
test('FFT physical report retains all30 attempts with exact PCM and independently checked CPU medians',()=>{
 const r=read(path.join(dir,'comparison.json')),pair=require('../tools/esp8266_opus_asm/report_fft_load3.cjs').verifyPair();assert.deepEqual(r.pair,pair);
 ...
 assert.deepEqual(r.initial.reference.timing_window_excesses.map(x=>[x.run,x.id,x.excess_us]),[[1,0,1314],[7,0,507]]);
 assert.deepEqual(r.repeated.reference.timing_window_excesses.map(x=>[x.run,x.id,x.excess_us]),[[3,0,157]]);
