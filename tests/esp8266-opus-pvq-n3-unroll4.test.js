...
const f=require('../tools/esp8266_opus_asm/pvq_n3_unroll4.cjs'),sem=require('../tools/esp8266_opus_asm/pvq_n3_unroll4_proof.cjs');
...
test('linked N3 differences match every U-row interval without RAM or stack growth',()=>{
...
test('unrolling changes only the helper relative to the scalar N3 experiment',()=>{
 const p=evidence(),scalar=require('../tools/esp8266_opus_asm/pvq_n3_diff.cjs');
 const old=scalar.read(path.join(scalar.art('candidate'),'preflight.json'));
 assert.deepEqual(p.patches[0],old.patches[0]);
 assert.equal(p.parent_elf_sha256,old.parent_elf_sha256);
 assert.equal(p.patches[1].address,old.patches[1].address);
 assert.equal(p.patches[1].bytes,old.patches[1].bytes);
 assert.notEqual(p.patches[1].after_hex,old.patches[1].after_hex);
 assert.equal(p.semantic.helper_instructions,23);
});
test('wrong branch, delta, subtraction and return mutations are rejected',()=>{
 const p=evidence();sem.checkHelper(p.helperDisassembly);
 for(const[from,to]of[['bltu','bgeu'],['bgeu','bltu'],['slli','srli'],['a12, a12, -2','a12, a12, -1'],['a12, a12, -3','a12, a12, -2'],['a12, a12, -4','a12, a12, -3'],['a6, a6, -4','a6, a6, -8'],['a8, a8, a6','a8, a6, a8'],['ret.n','retw.n']]){
  assert.ok(p.helperDisassembly.includes(from));assert.throws(()=>sem.checkHelper(p.helperDisassembly.replace(from,to)));
 }
 for(const[from,to]of[['call0','callx0'],['beqi','bnei'],['a15, a15, -4','a15, a15, -8'],['a3, a4, a5','a0, a4, a5']])
...
test('host semantic mirror passes24 exact PCM/state/sanitizer cases without bitrate cap',()=>{
 const r=f.read(path.join(f.art('candidate'),'correctness.json'));assert.equal(r.passed,true);assert.equal(r.kind,'pvq-n3-unroll4');
 assert.equal(r.recipe_sha256_lf,require('../tools/esp8266_opus_asm/export.cjs').sourceHash(require.resolve('../tools/esp8266_opus_asm/pvq_n3_unroll4.cjs')));
