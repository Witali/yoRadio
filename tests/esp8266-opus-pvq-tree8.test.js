...
const f=require('../tools/esp8266_opus_asm/pvq_tree8.cjs'),sem=require('../tools/esp8266_opus_asm/pvq_tree8_proof.cjs');
...
test('linked static tree search matches every U-row interval without RAM or stack growth',()=>{
 ...
 assert.ok(p.semantic.cases>100000);assert.equal(p.semantic.helper_instructions_covered,43);assert.equal(p.semantic.stores,0);assert.equal(p.semantic.sar_writes,0);
...
test('branch, leaf offset, load width and wrong cached value mutations are rejected',()=>{
 const p=evidence();sem.checkHelper(p.helperDisassembly);
 for(const[from,to]of[['bltu','bgeu'],['a6, a15, -28','a6, a15, -24'],['l32i.n','l16ui'],['a8, a3','a8, a9'],['ret.n','retw.n']]){
...
test('decoder-dead storage contract is revalidated; all other ELF bytes stay exact',()=>{
 ...
 assert.equal(p.storage.contract.caller.argument,'a2=0');assert.equal(p.storage.storage.bytes,123);
 assert.equal(p.patches.length,2);assert.equal(p.patches[0].address,sem.site);assert.equal(p.patches[1].address,sem.storageAddress);
 base.patchElf(a,p.patches);
});
test('host semantic mirror passes24 exact PCM/state/sanitizer cases without bitrate cap',()=>{
 const r=f.read(path.join(f.art('candidate'),'correctness.json'));assert.equal(r.passed,true);assert.equal(r.kind,'pvq-tree8');
 assert.equal(r.recipe_sha256_lf,require('../tools/esp8266_opus_asm/export.cjs').sourceHash(require.resolve('../tools/esp8266_opus_asm/pvq_tree8.cjs')));
