...
const root=path.resolve(__dirname,'..'),dir=path.join(root,'firmware/development/esp8266-opus-pvq-prefix-word-candidate-v1');
...
test('report refuses intermediate producer snapshots before archival',()=>{
 const {assertCompleteRun}=require('../tools/esp8266_opus_asm/report_pvq_prefix_word.cjs');
...
test('small RAM reserve is retained although PCM is exact and the speed candidate is rejected',()=>{
 const r=read(path.join(dir,'after/run10.json'));
 assert.equal(r.final.state,3);assert.equal(r.final.error,0);
 assert.equal(Math.min(...r.final.results.map(x=>x.min_dram)),376);
 assert.ok(r.final.results.every(x=>x.stack_free_lifetime===1660&&x.error===0));
 const c=result();assert.equal(c.inputs.after.length,10);assert.equal(c.accepted_for_experimental_asm,false);
 assert.equal(c.selection.initial.accepted_for_experimental_asm,false);assert.equal(c.selection.repeated.accepted_for_experimental_asm,false);
});
...
test('linked instruction census reproduces the recorded real searches',()=>{
 ...
 const replay=require('../tools/esp8266_opus_asm/analyze_pvq_prefix_word_instructions.cjs').analyze();
 assert.deepEqual(replay,saved);
 const c=saved.cases.find(c=>c.name==='stereo-192');assert.equal(c.old_instructions,119961);assert.equal(c.min_instructions,111543);assert.equal(c.max_instructions,122784);
