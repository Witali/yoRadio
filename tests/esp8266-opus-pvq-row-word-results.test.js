...
const {root,hash}=require('../tools/esp8266_opus_asm/export.cjs'),f=require('../tools/esp8266_opus_asm/pvq_row_word.cjs');
...
test('pvq-row-word PVQ physical report retains all30 attempts with exact PCM and independently checked CPU medians',()=>{
 const r=read(path.join(dir,'comparison.json')),pair=require('../tools/esp8266_opus_asm/report_pvq_row_word.cjs').verifyPair();assert.deepEqual(r.pair,pair);
...
test('pvq-row-word PVQ experiment restores ordinary radio OTA with matching station, playlist, HTTP and WebSocket',()=>{
