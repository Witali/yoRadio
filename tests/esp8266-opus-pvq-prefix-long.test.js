...
test('linked long-only packed prefix search matches every U-row interval without RAM or stack growth',()=>{
 ...
 assert.ok(p.semantic.guard.prefix_cases>0);assert.ok(p.semantic.guard.fallback_cases>0);assert.ok(p.semantic.guard.equal_cases>0);
});
...
test('branch, extraction, word width and SAR restoration mutations are rejected',()=>{
 ...
 for(const[from,to]of[['call0','callx0'],['bgei','blti'],['a15, a15, -4','a15, a15, -8'],['a3, a4, a5','a0, a4, a5'],['a3, a15, -16','a3, a15, -12'],['a2, a8, 40253332','a2, a8, 40253351']])
...
test('host semantic mirror passes24 exact PCM/state/sanitizer cases without bitrate cap',()=>{
 const r=f.read(path.join(f.art('candidate'),'correctness.json'));assert.equal(r.passed,true);assert.equal(r.kind,'pvq-prefix-long');
 assert.equal(r.recipe_sha256_lf,require('../tools/esp8266_opus_asm/export.cjs').sourceHash(require.resolve('../tools/esp8266_opus_asm/pvq_prefix_long.cjs')));
