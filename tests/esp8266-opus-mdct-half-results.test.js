...
const {root,hash}=require('../tools/esp8266_opus_asm/export.cjs'),f=require('../tools/esp8266_opus_asm/mdct_half.cjs');
const {compare}=require('../tools/esp8266_opus_profile/compare_raw.cjs'),{selectHighBitrate}=require('../tools/esp8266_opus_asm/selection.cjs');
const {validateRun}=require('../tools/esp8266_opus_asm/report_small_div_tail.cjs');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,'')),dir=f.art('candidate');
test('all thirty MDCT halfword measurements are retained, exact and reproducible',()=>{
 const r=read(path.join(dir,'comparison.json')),pair=require('../tools/esp8266_opus_asm/report_mdct_half.cjs').verifyPair();assert.deepEqual(r.pair,pair);
...
test('ordinary firmware is restored by OTA with WebSocket status and unchanged playlist',()=>{
 ...
 const ordinary=path.join(root,'firmware/development/esp8266-opus-live512-idle3s-20260913/app.bin');assert.equal(ota.pass,true);assert.equal(ota.sha256,hash(fs.readFileSync(ordinary)));assert.equal(after.status.app_address,ota.after.app_address);
 ...
 assert.equal(after.index.value,'getindex=1');assert.equal(after.index.messages.find(m=>Number.isInteger(m.current)).current,before.index.messages.find(m=>Number.isInteger(m.current)).current);
 assert.ok(after.index.messages.some(m=>m.payload?.some(p=>p.id==='playerwrap'&&p.value==='stopped')));assert.deepEqual(after.playlist.wire_sha256,before.playlist.wire_sha256);assert.ok(after.playlist.wire_bytes>0);
