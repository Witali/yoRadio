...
test('production direct DMA writer preserves PCM/PDM across chunk boundaries and EOF races', t => {
  ...
  fs.writeFileSync(path.join(dir, 'output.inc'), helpers + output.slice(
    output.indexOf('\n', start) + 1, end));
  for(const mode of ['PDM32', 'PDM32_CLOCK', 'RCPDM', 'RCPDM_SIMPLE', 'RCPDM_FEEDBACK', 'PDM128']) {
   for(const dmaWords of (mode.startsWith('PDM32') ? [128, 192, 256, 512, 768] : [512])) {
    ...
    if(process.platform === 'win32') {
      ...
      fs.writeFileSync(batch, `@call "${vcvars}" >nul\r\n@if errorlevel 1 exit /b %errorlevel%\r\n@cl /nologo /std:c++20 /O2 /EHsc /W3 /DTEST_${mode}=1 /DTEST_DMA_WORDS=${dmaWords}${queueDefine} /I"${source}" /I"${audio}" /I"${dir}" ${files.map(p => `"${p}"`).join(' ')} /Fe:"${exe}"\r\n`);
      build = spawnSync('cmd.exe', ['/d', '/c', batch], {cwd:dir, encoding:'utf8'});
    } else {
      build = spawnSync('c++', ['-std=c++20', '-O2', `-DTEST_${mode}=1`, `-DTEST_DMA_WORDS=${dmaWords}`, ...(dmaWords<=256?['-DYORADIO_ESP8266_OPUS_PCM_QUEUE=1']:[]), `-I${source}`, `-I${audio}`, `-I${dir}`, ...files, '-o', exe], {cwd:dir, encoding:'utf8'});
