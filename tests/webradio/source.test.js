...
const hardwareTest = fs.readFileSync(path.join(__dirname, 'hardware-playback.mjs'), 'utf8');
...
test('ESP8266Audio reference target uses the official streaming pipeline', () => {
  ...
  assert.match(sketch, /constexpr size_t kStreamBufferBytes = 4 \* 1024/);
  assert.match(sketch, /constexpr size_t kCodecWorkspaceBytes = 29192/);
});

test('ESP8266Audio Web API exposes status, diagnostics and player controls', () => {
  ...
  for (const field of ['samples', 'sample_rate', 'channels']) {
    assert.ok(sketch.includes(`\\"${field}\\"`), `missing ${field} playback diagnostic`);
  }
  assert.match(sketch, /class MeteredAudioOutputI2SNoDAC/);
  assert.match(sketch, /AudioOutputI2SNoDAC::ConsumeSample\(sample\)/);
});
...
test('physical playback test uses local reproducible MP3 and AAC fixtures', () => {
  assert.match(hardwareTest, /mono-64\.mp3/);
  assert.match(hardwareTest, /mono-64\.aac/);
  assert.match(hardwareTest, /\/stream\.mp3/);
  assert.match(hardwareTest, /\/stream\.aac/);
  assert.match(hardwareTest, /sample count did not advance/);
});
