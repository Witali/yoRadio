...
const root=path.resolve(__dirname,'..'),dir=path.join(root,'firmware/development/esp8266-opus-pvq-prefix-long-candidate-v1');
...
test('report refuses intermediate producer snapshots before archival',()=>{
 const {assertCompleteRun}=require('../tools/esp8266_opus_asm/report_pvq_prefix_long.cjs');
...
test('slow control and tight candidate heap remain included, not hidden by median selection',()=>{
 const slow=read(path.join(dir,'after/run6.json')),tight=read(path.join(dir,'candidate/run2.json'));
 assert.equal(slow.final.state,3);assert.equal(slow.final.error,0);
 assert.ok(slow.snapshots.some(s=>s.error==='request timeout'));
 assert.equal(slow.final.results[4].task_us,2159122);assert.equal(slow.final.results[4].max_wall_us,28183);
 assert.equal(Math.min(...tight.final.results.map(r=>r.min_dram)),396);
 assert.ok(tight.final.results.every(r=>r.stack_free_lifetime===1660&&r.error===0));
 const r=result();assert.equal(r.inputs.after.length,10);assert.equal(r.inputs.candidate.length,10);
 assert.equal(r.accepted_for_experimental_asm,false);assert.equal(r.selection.initial.accepted_for_experimental_asm,false);
...
test('linked instruction census reproduces the recorded real searches',()=>{
 ...
 const replay=require('../tools/esp8266_opus_asm/analyze_pvq_prefix_long_instructions.cjs').analyze();
 assert.deepEqual(replay,saved);
 const c=saved.cases.find(c=>c.name==='stereo-192');assert.equal(c.old_instructions,119961);assert.equal(c.min_instructions,122889);assert.equal(c.max_instructions,128848);
 assert.equal(c.old_U_reads,24259);
 assert.equal(c.min_U_reads,17422);assert.equal(c.max_U_reads,18802);
 assert.equal(c.min_total_reads,20034);assert.equal(c.max_total_reads,21414);
 assert.equal(c.literal_reads,1306);assert.equal(c.prefix_word_reads,1306);
 assert.equal(c.long_guard.hypothetical,false);assert.equal(c.long_guard.extra_U_reads,3159);
});
