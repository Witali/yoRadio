...
const root=path.resolve(__dirname,'..'),dir='firmware/development/esp8266-opus-ebands-u16-aligned-candidate-v1';
...
test('ebands-u16-aligned A/B/A recomputes every raw trial and the unrounded75% gate',()=>{
 ...
 assert.equal(r.current_cpu_target.raw_cpu_target_met,false);
...
test('Additional ebands-u16-aligned experiment restores the previous ordinary image with saved settings and playlist',()=>{
 const before=read(dir+'/initial.json'),after=read(dir+'/restored.json'),ota=read(dir+'/ota-restore.json'),ordinary='firmware/development/esp8266-opus-live-asm-ebands-final-20260917';
