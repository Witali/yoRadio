...
const {root,hash}=require('../tools/esp8266_opus_asm/export.cjs'),f=require('../tools/esp8266_opus_asm/ec_bits.cjs');
...
test('ec_dec_bits physical report retains all30 attempts with exact PCM and independently checked CPU medians',()=>{
 const r=read(path.join(dir,'comparison.json')),pair=require('../tools/esp8266_opus_asm/report_ec_bits.cjs').verifyPair();assert.deepEqual(r.pair,pair);
 ...
 assert.deepEqual(r.repeated.reference.timing_window_excesses.map(v=>[v.run,v.id,v.excess_us]),[[4,0,341]]);
 for(const g of[r.initial.reference,r.initial.candidate,r.repeated.reference])assert.deepEqual(g.observation_errors,[]);
 assert.equal(Math.min(...r.initial.candidate.cases.map(c=>c.min_dram.min)),8160);
...
test('ec_dec_bits experiment restores ordinary radio OTA with matching station, playlist, HTTP and WebSocket',()=>{
