...
const {root,hash}=require('../tools/esp8266_opus_asm/export.cjs'),f=require('../tools/esp8266_opus_asm/fft_schedule.cjs');
...
test('FFT physical report retains all30 attempts with exact PCM and independently checked CPU medians',()=>{
 const r=read(path.join(dir,'comparison.json')),pair=require('../tools/esp8266_opus_asm/report_fft_schedule.cjs').verifyPair();assert.deepEqual(r.pair,pair);
 ...
 for(const k of['initial','repeated']){assert.deepEqual(selectHighBitrate(r[k].cases),r.selection[k]);assert.equal(r.selection[k].accepted_for_experimental_asm,false);assert.equal(r.selection[k].target_192_cpu_at_most_70,false);}
 assert.deepEqual(r.initial.reference.timing_window_excesses.map(x=>[x.run,x.id,x.excess_us]),[[5,0,2603]]);
 assert.deepEqual(r.repeated.reference.timing_window_excesses.map(x=>[x.run,x.id,x.excess_us]),[]);
 assert.deepEqual(r.initial.candidate.timing_window_excesses.map(x=>[x.run,x.id,x.excess_us]),[[6,0,1236],[8,0,2898]]);
 for(const summary of[r.initial.reference,r.initial.candidate,r.repeated.reference])assert.deepEqual(summary.observation_errors,[]);
...
test('FFT experiment restores ordinary radio OTA with matching station, playlist, HTTP and WebSocket',()=>{
