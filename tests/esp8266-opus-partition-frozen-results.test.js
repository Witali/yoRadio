...
const {root,hash}=require('../tools/esp8266_opus_asm/export.cjs'),f=require('../tools/esp8266_opus_asm/partition_frozen.cjs');
...
test('Frozen partition physical report retains all30 attempts with exact PCM and independently checked CPU medians',()=>{
 const r=read(path.join(dir,'comparison.json')),pair=require('../tools/esp8266_opus_asm/report_partition_frozen.cjs').verifyPair();assert.deepEqual(r.pair,pair);
 ...
 assert.equal(groups.candidate[2].final.dram_after,26652);
 assert.equal(groups.candidate[2].final.results[4].min_dram,10152);
 assert.deepEqual(r.initial.reference.timing_window_excesses.map(v=>[v.run,v.id,v.excess_us]),[[9,0,1751]]);
 for(const g of[r.initial.reference,r.initial.candidate,r.repeated.reference])assert.deepEqual(g.observation_errors,[]);
 const h=read(path.join(dir,'host-parent.json'));assert.equal(h.passed,true);assert.equal(h.cases.length,24);assert.equal(h.compound.length,3);assert.ok(h.cases.some(c=>c.name==='stereo-510'));assert.ok(h.cases.every(c=>c.pcm.exact&&c.pcm.max_absolute_error===0));
});
test('Frozen partition experiment restores ordinary radio OTA with matching station, playlist, HTTP and WebSocket',()=>{
