...
int main() {
    try {
        uint64_t count=0;uint32_t random=8266;
        auto one=[&](rc_pdm_feedback_t &a,rc_pdm_feedback_t &b,int16_t sample,
            ...
            rc_pdm_feedback_frame(&a,sample,got,bits,shift,gain,dither,interp);
            rc_feedback_reference_frame(&b,sample,want,bits,shift,gain,dither,interp);
            check(std::equal(got,got+4,want)&&same(a,b),"word/state reference mismatch");
