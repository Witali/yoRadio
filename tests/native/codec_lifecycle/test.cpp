...
static std::vector<size_t> allocation_sizes;
// Bounded first-fit fragmentation model, not a timing/SDK allocator emulator.
static std::vector<size_t> free_chunks;
static std::map<void *, size_t> chunk_owners;
static size_t rounded(size_t n) { return (n + 15u) & ~size_t(15u); }
static size_t take_chunk(std::vector<size_t> &chunks, size_t bytes) {
    for (size_t i=0;i<chunks.size();++i) if (chunks[i]>=rounded(bytes)) {
        chunks[i]-=rounded(bytes); return i;
    }
    return chunks.size();
}
void *heap_caps_malloc(size_t size, unsigned caps) {
    ...
    size_t chunk=free_chunks.size();
    if ((caps & MALLOC_CAP_8BIT) && !free_chunks.empty()) {
        chunk=take_chunk(free_chunks,size);
        if (chunk==free_chunks.size()) { ++failures; return nullptr; }
    }
    void *p = malloc(size); assert(p);
    assert(live.emplace(p, size).second); ++allocations;
    assert(live_caps.emplace(p,caps).second);
    if (chunk<free_chunks.size()) assert(chunk_owners.emplace(p,chunk).second);
    if (record_allocations && (caps & MALLOC_CAP_8BIT)) allocation_sizes.push_back(size);
    return p;
}
...
void heap_caps_free(void *p) {
    assert(!critical_depth);
    if (!p) return;
    if (chunk_owners.count(p)) {
        free_chunks.at(chunk_owners.at(p))+=rounded(live.at(p));
        chunk_owners.erase(p);
    }
...
#if CONFIG_YORADIO_OGG_OPUS
static void opus_fragmented_allocation(const std::map<void *, size_t> &baseline) {
    allocation_sizes.clear(); record_allocations=true;
    helix_codec_t *c=helix_codec_create(HELIX_CODEC_OPUS,1152); assert(c);
    record_allocations=false;
    assert(allocation_sizes.size()==6);
    assert(allocation_sizes[3]==native_opus_decoder_size());
    assert(allocation_sizes[4]==CONFIG_YORADIO_OPUS_SCRATCH_BYTES);
    const size_t workspace=allocation_sizes[5];
    assert(workspace<4096 && allocation_sizes[4]>4096);
    helix_codec_destroy(c); clean(baseline);
    size_t large=64;
    for (size_t i=0;i<5;++i) large+=rounded(allocation_sizes[i]);
    const std::vector<size_t> layout={large,rounded(workspace)+64,4096};
    auto old_chunks=layout;
    const size_t old_order[]={0,1,2,5,3,4};
    size_t completed=0;
    for (auto i:old_order) {
        if (take_chunk(old_chunks,allocation_sizes[i])==old_chunks.size()) break;
        ++completed;
    }
    assert(completed==5); // The old ordering fails exactly at scratch.
    for (int cycle=0;cycle<10;++cycle) {
        free_chunks=layout;
        c=helix_codec_create(HELIX_CODEC_OPUS,1152); assert(c);
        capacity_and_owner(c,HELIX_CODEC_OPUS);
        const auto allocated=free_chunks;
        assert(helix_codec_switch(c,HELIX_CODEC_OPUS)==0 && free_chunks==allocated);
        helix_codec_destroy(c); clean(baseline);
        assert(free_chunks==layout && chunk_owners.empty());
        free_chunks.clear();
    }
    puts("Opus large-first fragmentation: old scratch OOM, new exact-size fit, 10 reset/free cycles PASS");
}
...
static void opus_diagnostics(const std::map<void *, size_t> &baseline) {
    const unsigned stages[]={HELIX_OPUS_INIT_CODEC,HELIX_OPUS_INIT_INPUT,HELIX_OPUS_INIT_PCM,
        HELIX_OPUS_INIT_STATE,HELIX_OPUS_INIT_SCRATCH,HELIX_OPUS_INIT_WORKSPACE};
...
int main(int argc, char **argv) {
    ...
    opus_fragmented_allocation(baseline);
