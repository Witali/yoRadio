...
const root=path.resolve(__dirname,'..'),dir=path.join(root,'firmware/development/esp8266-opus-pvq-binary-candidate-v1');
...
test('linked instruction census reproduces the recorded real searches',()=>{
 ...
 const replay=require('../tools/esp8266_opus_asm/analyze_pvq_binary_instructions.cjs').analyze();
 assert.deepEqual(replay,saved);
 const c=saved.cases.find(c=>c.name==='stereo-192');assert.equal(c.old_instructions,119961);assert.equal(c.new_instructions,237196);
 assert.equal(c.old_U_reads,24259);assert.equal(c.new_U_reads,17587);
});
