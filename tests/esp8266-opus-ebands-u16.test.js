...
const f=require('../tools/esp8266_opus_asm/ebands_u16.cjs'),s=require('../tools/esp8266_opus_asm/ebands_u16_proof.cjs');
...
test('immutable patch definitions and24 exact PCM host scenarios through510kbps and120ms',()=>{
 ...
 const h=read('host.json');assert.equal(h.passed,true);assert.equal(h.kind,'ebands-u16');assert.equal(h.cases.length,24);assert.equal(h.compound.length,3);
 ...
 assert.equal(h.recipe_sha256_lf,sourceHash(path.join(__dirname,'../tools/esp8266_opus_asm/ebands_u16.cjs')));
});
