...
const root=path.resolve(__dirname,'..'),dir='firmware/development/esp8266-opus-ebands-u16-candidate-v1';
...
test('ebands-u16 A/B/A recomputes every raw trial and the unrounded75% gate',()=>{
 ...
 assert.equal(r.selection.initial.accepted_for_experimental_asm,false);assert.equal(r.selection.repeated.accepted_for_experimental_asm,true);
 assert.equal(r.selection.initial.accepted_for_experimental_asm&&r.selection.repeated.accepted_for_experimental_asm,false,'Do not cherry-pick the successful repeated control');
...
test('Additional ebands-u16 experiment restores the previous ordinary image with saved settings and playlist',()=>{
