...
const {candidate,control}=require('../tools/esp8266_opus_asm/report_small_div_inline.cjs');
const dir=path.join(root,'firmware/development',candidate),read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
test('inline division archive retains all 30 actual A/B/A measurements and maxima',()=>{
