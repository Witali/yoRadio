...
const root=path.resolve(__dirname,'..'),dir=path.join(root,'firmware/development/esp8266-opus-pvq-tree8-candidate-v1');
...
test('report refuses intermediate producer snapshots before archival',()=>{
 const {assertCompleteRun}=require('../tools/esp8266_opus_asm/report_pvq_tree8.cjs');
 ...
 const partial=read(path.join(dir,'report-incomplete/run10.json'));
 assert.throws(()=>assertCompleteRun(partial));
});
...
test('linked instruction census reproduces the recorded real searches',()=>{
 ...
 const replay=require('../tools/esp8266_opus_asm/analyze_pvq_tree8_instructions.cjs').analyze();
 assert.deepEqual(replay,saved);
 const c=saved.cases.find(c=>c.name==='stereo-192');assert.equal(c.old_instructions,119961);assert.equal(c.new_instructions,105794);
 assert.equal(c.old_U_reads,24259);assert.equal(c.new_U_reads,18633);
});
