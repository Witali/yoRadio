...
const f=require('../tools/esp8266_opus_asm/pvq_prefix_word.cjs'),sem=require('../tools/esp8266_opus_asm/pvq_prefix_word_proof.cjs');
...
test('word representation retains every packed bound and identical site instructions',()=>{
 const p=evidence(),old=require('../tools/esp8266_opus_asm/pvq_prefix0.cjs');
 const packed=require('../tools/esp8266_opus_asm/pvq_prefix0_proof.cjs').prefix(p.table),words=sem.prefix(p.table);
 assert.equal(words.length,packed.length*4);
 for(let i=0;i<packed.length;i++)assert.equal(words.readUInt32LE(i*4),packed[i]);
 assert.equal(p.patches[0].after_hex,old.read(path.join(old.art('candidate'),'preflight.json')).patches[0].after_hex);
});
test('linked direct-word prefix search matches every U-row interval without RAM or stack growth',()=>{
 ...
 assert.ok(p.semantic.cases>100000);assert.equal(p.semantic.helper_instructions_covered,18);assert.equal(p.semantic.stores,0);assert.equal(p.semantic.sar_preserved,true);
...
test('branch, index scaling and word width mutations are rejected',()=>{
 const p=evidence();sem.checkHelper(p.helperDisassembly);
 for(const[from,to]of[['bltu','bgeu'],['a3, a13, 5','a3, a13, 4'],['l32i.n','l16ui'],['addx4','addx2'],['nsau','nsa'],['ret.n','retw.n']]){
...
test('host semantic mirror passes24 exact PCM/state/sanitizer cases without bitrate cap',()=>{
 const r=f.read(path.join(f.art('candidate'),'correctness.json'));assert.equal(r.passed,true);assert.equal(r.kind,'pvq-prefix-word');
 assert.equal(r.recipe_sha256_lf,require('../tools/esp8266_opus_asm/export.cjs').sourceHash(require.resolve('../tools/esp8266_opus_asm/pvq_prefix_word.cjs')));
...
test('word bounds are exact flash bytes; pointer and byte mutations fail closed',()=>{
 const p=evidence(),a=original(),b=base.patchElf(a,p.patches);
 assert.deepEqual(sem.prefixProof(b,p.table),p.prefix_table);assert.equal(p.prefix_table.bytes,1408);
 assert.doesNotMatch(p.helperDisassembly,/\b(?:ssr|srl|rsr\.sar|wsr\.sar|extui)\b/);
...
test('full encoder storage is unreachable and has no address-taken references',()=>{
 ...
 assert.equal(x.address,sem.dataAddress);assert.equal(x.bytes,1412);assert.equal(x.function_bytes,1788);
