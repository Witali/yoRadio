...
const root=path.resolve(__dirname,'..'),dir=path.join(root,'firmware/development/esp8266-opus-pvq-n3-unroll4-candidate-v1');
...
test('N3 report refuses intermediate snapshots before immutable archival',()=>{
 const {assertCompleteRun}=require('../tools/esp8266_opus_asm/report_pvq_n3_unroll4.cjs');
...
test('linked instruction census matches the independent finite-difference model',()=>{
 ...
 assert.deepEqual(require('../tools/esp8266_opus_asm/analyze_pvq_n3_unroll4_instructions.cjs').analyze(),saved);
 const c=saved.cases.find(c=>c.name==='stereo-192');
 assert.equal(c.old_instructions,119961);assert.equal(c.new_instructions,122440);assert.equal(c.old_U_reads,24259);assert.equal(c.new_U_reads,18357);
 const proof=result().pair.proof;
 assert.equal(proof.semantic.helper_instructions,23);assert.equal(proof.patches.length,2);
