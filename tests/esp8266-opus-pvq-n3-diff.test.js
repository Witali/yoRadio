...
const f=require('../tools/esp8266_opus_asm/pvq_n3_diff.cjs'),sem=require('../tools/esp8266_opus_asm/pvq_n3_diff_proof.cjs');
...
test('wrong branch, delta, subtraction and return mutations are rejected',()=>{
 ...
 for(const[from,to]of[['bltu','bgeu'],['slli','srli'],['a6, a6, -4','a6, a6, -8'],['a8, a8, a6','a8, a6, a8'],['ret.n','retw.n']]){
...
test('host semantic mirror passes24 exact PCM/state/sanitizer cases without bitrate cap',()=>{
 const r=f.read(path.join(f.art('candidate'),'correctness.json'));assert.equal(r.passed,true);assert.equal(r.kind,'pvq-n3-diff');
 assert.equal(r.recipe_sha256_lf,require('../tools/esp8266_opus_asm/export.cjs').sourceHash(require.resolve('../tools/esp8266_opus_asm/pvq_n3_diff.cjs')));
