...
const {root,hash}=require('../tools/esp8266_opus_asm/export.cjs'),f=require('../tools/esp8266_opus_asm/endpoint_cost.cjs');
...
test('endpoint-cost PVQ physical report retains all30 attempts with exact PCM and independently checked CPU medians',()=>{
 const r=read(path.join(dir,'comparison.json')),pair=require('../tools/esp8266_opus_asm/report_endpoint_cost.cjs').verifyPair();assert.deepEqual(r.pair,pair);
...
test('endpoint-cost PVQ experiment restores ordinary radio OTA with matching station, playlist, HTTP and WebSocket',()=>{
