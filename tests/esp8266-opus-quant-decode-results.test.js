...
const result=()=>read(path.join(root,'firmware/development/esp8266-opus-quant-decode-candidate-v2/comparison.json'));
...
test('all fresh and preliminary physical attempts match hashes, PCM and confirmed OTA slots',()=>{
 ...
 for(const group of ['before','candidate','after']){
  ...
  const ota=read(path.join(root,'firmware/development/esp8266-opus-quant-decode-candidate-v2/ota-'+group+'.json'));
 ...
 const preliminary=r.preliminary_control;
 assert.equal(preliminary.inputs.length,10);assert.match(preliminary.reason,/memory audit/);
 const oldOta=read(path.join(root,'firmware/development/esp8266-opus-quant-decode-candidate-v2/ota-preliminary-control.json'));
 assert.equal(oldOta.pass,true);assert.equal(oldOta.sha256,r.pair.manifests.control.app_sha256);
 for(const item of preliminary.inputs){
  assert.ok(!seen.has(item.file));seen.add(item.file);
  const bytes=fs.readFileSync(path.join(root,item.file));
  assert.equal(crypto.createHash('sha256').update(bytes).digest('hex'),item.sha256);
  validateRun(JSON.parse(bytes),fixtures,oldOta.after.app_address);
 }
...
test('completed benchmark is followed by confirmed ordinary-radio OTA and WebUI responses',()=>{
 const dir=path.join(root,'firmware/development/esp8266-opus-quant-decode-candidate-v2');
 const before=read(path.join(dir,'before-restore.json'));
 assert.equal(before.state,3);assert.equal(before.error,0);
 ...
 assert.equal(ota.before.app_address,0x110000);assert.equal(ota.after.app_address,0x10000);
 const ui=read(path.join(dir,'restored-webui.json'));
 assert.equal(ui.status.app_address,ota.after.app_address);
 assert.equal(ui.status.firmware,'esp8266-native');assert.equal(ui.status.playing,false);
 assert.equal(ui.status.error,'');assert.ok(ui.audio);
