...
const result=()=>read(path.join(root,'firmware/development/esp8266-opus-allocation-decode-candidate-v1/comparison.json'));
...
test('all thirty unique physical attempts match hashes, PCM and confirmed OTA slots',()=>{
 ...
 for(const group of ['before','candidate','after']){
  ...
  const ota=read(path.join(root,'firmware/development/esp8266-opus-allocation-decode-candidate-v1/ota-'+group+'.json'));
...
test('completed benchmark is followed by confirmed ordinary-radio OTA and WebUI responses',()=>{
 const dir=path.join(root,'firmware/development/esp8266-opus-allocation-decode-candidate-v1');
